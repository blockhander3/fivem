...
local handsUpButton				= 0 --- H by default -- use 73 for X
...
RegisterKeyMapping('ce handsup', '[ANIMÁCIÓ] Kézfeltétel', 'keyboard', '')
...
RegisterCommand("ce",function(source, args)
	...
	if tostring(args[1]) == nil then
		...
		if tostring(args[1]) ~= nil then
			...
			elseif argh == 'caridle1' then
				local ad = "amb@code_human_in_car_idles@generic@ds@idle_a"
				...
				if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
					...
					if ( IsEntityPlayingAnim( player, ad, "idle_a", 3 ) ) then 
						...
						TaskPlayAnim( player, ad, "generic@ds@idle_a", 8.0, 1.0, -1, 49, 0, 0, 0, 0 )
					end     
				end
