...
-- Make sure this only runs on blckhndr_main
-- Allows for @blckhndr_main/debug/sh_debug.lua in __resource.lua
if GetCurrentResourceName() == "blckhndr_main" then
	RegisterCommand("blckhndr_debug", function(src, args, raw)
		if args[1] == "true" or args[1] == false then
			TriggerEvent("blckhndr_main:debug_toggle", args[1] == "true")
		else
			print("'blckhndr_debug' = "..tostring(Debug.IsEnabled))
			print('Type: Boolean, Values: "false", "true"')
		end
	end)
end

AddEventHandler("blckhndr_main:debug_toggle", function(debug)
	Debug.IsEnabled = debug
end)

-- Print except only available with "blckhndr_debug true"
