...
function checkBan(source, setKickReason)
	...
	for k, v in pairs(GetPlayerIdentifiers(src)) do
		--print('>>>    :blckhndr_main: (sv_bans.lua) Checking bans for Player('..src..'), Identifier('..v..')')
		sql = "SELECT * FROM `blckhndr_bans` WHERE `ban_identifier` = '"..v.."'"
		--print('>>>>>>>>>>> '..sql)
		...
		if check[1] then
			if check[1].ban_expire >= os.time() or check[1].ban_expire == -1 then
				...
				if check[1]['ban_expire'] == -1 then
					print(':blckhndr_main: (sv_bans.lua) - Player('..src..') is PERM banned, Identifier('..v..') - dropping player.')
					...
					print(':blckhndr_main: (sv_bans.lua) - Player('..src..') is banned until Date('..check[1]['ban_expire']..'), Identifier('..v..') - dropping player.')
...
function updateIdentifiers(src)
	...
	if not steamid then print('trying to update identifiers for player('..src..') failed') return else print(':blckhndr_main: (sv_bans.lua) Updating identifiers for Player('..src..') with SteamID('..steamid..')') end
	local sql = MySQL.Sync.fetchAll("SELECT * FROM `blckhndr_users` WHERE `steamid` = '"..steamid.."'")
	if sql[1] then
		...
		--print(':blckhndr_main: Updating user_id('..sql[1]['user_id']..') identifiers\n to '..update)
		
		MySQL.Sync.execute("UPDATE `blckhndr_users` SET `identifiers` = '"..update.."' WHERE `user_id` = "..sql[1]['user_id']..";")
	end
