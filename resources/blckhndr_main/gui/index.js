  ...
  window.addEventListener('message', function(event) {
	...
	if (event.data.type == 'characterInfo') {
		if (event.data.char1_id != 'nothing') {
			...
			$('#char1_id').html('<b>Karakter ID:</b> ')
			$('#char1_fname').html('<b>Keresztnév:</b> ')
			$('#char1_lname').html('<b>Vezetéknév:</b> ')
			$('#char1_dob').html('<b>Születési dátum:</b> ')
		...
		if (event.data.char2_id != 'nothing') {
			...
			$('#char2_id').html('<b>Karakter ID:</b> ')
			$('#char2_fname').html('<b>Keresztnév:</b> ')
			$('#char2_lname').html('<b>Vezetéknév:</b> ')
			$('#char2_dob').html('<b>Születési dátum:</b> ')
		...
		if (event.data.char3_id != 'nothing') {
			...
			$('#char3_id').html('<b>Karakter ID:</b> ')
			$('#char3_fname').html('<b>Keresztnév:</b> ')
			$('#char3_lname').html('<b>Vezetéknév:</b> ')
			$('#char3_dob').html('<b>Születési dátum:</b> ')
...
document.body.onmouseup = function() {
	...
	if ($('.createcreatebutton:hover').length != 0) {
		if ($('#create-fname').val() && $('#create-lname').val() && $('#create-dob').val()) {
			...
			$.post('http://blckhndr_main/createCharacter', JSON.stringify({
	...
	if ($('#select_1:hover').length != 0) {
		...
		$.post('http://blckhndr_main/spawnCharacter', JSON.stringify({
