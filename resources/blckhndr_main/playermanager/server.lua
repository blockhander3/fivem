...
AddEventHandler('playerConnecting', function(playername, setKickReason)
  ...
  if mysql then
    ...
    MySQL.Async.fetchAll("SELECT * FROM blckhndr_users WHERE steamid = '"..identity[1].."'", {}, function(user)
      if not user[1] then
	playername = playername:gsub('%W','')
        MySQL.Sync.execute("INSERT INTO `blckhndr_users` (`name`, `steamid`, `connections`, `banned`, `banned_r`) VALUES ('"..playername.."', '"..identity[1].."', '1', '0', '')")
        ...
        if user[1].banned > os.time() or user[1].banned == -1 then
          ...
          MySQL.Sync.execute("UPDATE `blckhndr_users` SET `connections` = '"..connections.."' WHERE `blckhndr_users`.`steamid` = '"..identity[1].."';")
...
RegisterServerEvent('blckhndr_main:validatePlayer')
...
MySQL.ready(function()
    mysql = true
    MySQL.Async.execute('UPDATE `blckhndr_vehicles` SET `veh_status` = 0 WHERE `veh_status` = 1', {}, function() end)
end)
