...
fname2 = nil
lname2 = nil
RegisterNetEvent('playernames:names')
AddEventHandler('playernames:names', function(fname3, lname3, charid3)
fname = fname3
print(fname)
lname = lname3
charid = charid3
...
function formatPlayerNameTag(i, templateStr)
    ...
    for i,player in ipairs(GetActivePlayers()) do
     --[[   print(player)
        printout = player
    charid = exports.blckhndr_main:blckhndr_CharID(player)
    fname = exports.blckhndr_main:blckhndr_CharNamesF(player)
    lname = exports.blckhndr_main:blckhndr_CharNamesL(player)--]]
    TriggerServerEvent('blckhndr_main:getCharacterFromID', player)
    end
    --print(fname)
    printout = fname
