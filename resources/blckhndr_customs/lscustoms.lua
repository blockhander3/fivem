function moneycheck(amt)
	if amt == nil then
		 TriggerEvent('blckhndr_notify:displayNotification', '<b>SHIT</b> Something went wrong lol.', 'centerLeft', 3000, 'error')
		return false
	end
	local result = false

	 if tonumber(exports.blckhndr_main:blckhndr_GetWallet()) >= tonumber(amt) then
	 	return true
	 else
...
local lsc = {
	...
	locations = {
		...
		[1] = {outside = { x = 0.8386, y = 0.241, z = 31.5004, heading = 0.01}, inside = {x = 106.370262146, y = 6620.2016601562, z = 31.27465057373, heading = 0.01}},
...
function DriveOutOfGarage()
	...
	TriggerServerEvent('blckhndr_cargarage:updateVehicle', exports["blckhndr_cargarage"]:getCarDetails(veh))
end
				...
				if DoesEntityExist(veh) and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
					for i,pos in ipairs(lsc.locations) do
						...
						if GetDistanceBetweenCoords(pos.inside.x,pos.inside.y,pos.inside.z,GetEntityCoords(ped)) <= 10 then
							...
							Util.DrawText3D(pos.inside.x,pos.inside.y,pos.inside.z, 'Nyomj ~g~[ ENTER ] ~s~-t, hogy belépj a Los Santos Customs-be', false)
	...
	--[[if lsc.currentmenu == "repair" then
		if button.name == "Repair vehicle" then
			if (moneycheck(button.costs)) then
				...
				TriggerEvent('blckhndr_bank:change:walletMinus', button.costs)
				SetVehicleFixed(car)
				OpenMenu('main')
			end
		end--]]
