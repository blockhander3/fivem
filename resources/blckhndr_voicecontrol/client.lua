    ...
    if onphone then
      if _holding then
        ...
        if IsControlJustPressed(0,11) then
          TriggerServerEvent('blckhndr_voicecontrol:call:end', callwith)
        end
      elseif holding then
        SetTextComponentFormat("STRING")
        ...
        if IsControlJustPressed(0,10) then
          TriggerServerEvent('blckhndr_voicecontrol:call:unhold', callwith)
        end
        if IsControlJustPressed(0,11) then
          TriggerServerEvent('blckhndr_voicecontrol:call:end', callwith)
        end
        ...
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)
        if IsControlJustPressed(0,10) then
          TriggerServerEvent('blckhndr_voicecontrol:call:hold', callwith)
        end
        if IsControlJustPressed(0,11) then
          TriggerServerEvent('blckhndr_voicecontrol:call:end', callwith)
        end
...
RegisterNetEvent('blckhndr_voicecontrol:call:ring')
AddEventHandler('blckhndr_voicecontrol:call:ring', function(callfrom)
  local ringing = true
  Citizen.CreateThread(function()
    while true do
      Citizen.Wait(0)

        TriggerServerEvent('blckhndr_voicecontrol:call:answer', callfrom)
        ringing = false
    end
  end)
  Citizen.CreateThread(function()
    while true do
      Citizen.Wait(0)
      if ringing then
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3, 'blckhndr_phonecall.ogg', 0.4)
        Citizen.Wait(2500)
      else
        break
      end
