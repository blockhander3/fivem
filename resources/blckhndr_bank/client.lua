...
RegisterNetEvent('blckhndr_bank:change:bankandwallet')
RegisterNetEvent('blckhndr_bank:request:both')
RegisterNetEvent('blckhndr_bank:update:both')
AddEventHandler('blckhndr_bank:update:both', function(wallet, bank)
  moneys = wallet
  banks = bank
  SendNUIMessage({
    type = 'update',
      ...
      if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)).x, GetEntityCoords(GetPlayerPed(-1)).y, GetEntityCoords(GetPlayerPed(-1)).z, atm.x, atm.y, atm.z) < 1 then
        if not atmDisplay then
          ...
          if IsControlJustPressed(0,38) then
            TriggerEvent('blckhndr_bank:request:both')
...
RegisterNUICallback('depositMoney', function(tbl)
  if tbl.atbank == 'false' or tbl.atbank == false then
	blckhndr_closeATM()
	TriggerEvent('blckhndr_notify:displayNotification', 'You cannot deposit money at an ATM!', 'centerRight', 4000, 'error')
	return
  end
  if not tonumber(tbl.deposit) or tonumber(tbl.deposit) < 1 then
	blckhndr_closeATM()
	TriggerEvent('blckhndr_notify:displayNotification', 'There was an issue with your input!', 'centerRight', 4000, 'error')
	return
  end
  if tonumber(tbl.deposit) > 500000 then
	blckhndr_closeATM()
	TriggerEvent('blckhndr_notify:displayNotification', 'Maximum deposit of $500,000', 'centerRight', 4000, 'error')
	return
  end
  if tonumber(moneys) >= tonumber(tbl.deposit) then
	...
	TriggerEvent('blckhndr_phones:SYS:addTransaction', {
		title = 'Bank Deposit',
		trantype = 'CREDIT',
		systype = 'credit',
		tranamt = tbl.deposit
	})
    TriggerEvent('blckhndr_bank:change:bankandwallet', new_wallet, new_bank)
	TriggerServerEvent('blckhndr_main:logging:addLog', GetPlayerServerId(PlayerId()), 'money', 'Character('..exports["blckhndr_main"]:blckhndr_CharID(GetPlayerServerId(PlayerId()))..') deposited $'..tbl.deposit)
  else
	TriggerEvent('blckhndr_notify:displayNotification', 'You don\'t have enough money!', 'centerRight', 4000, 'error')
  end
  blckhndr_closeATM()
end)
...
RegisterNUICallback('withdrawMoney', function(tbl)
  ...
  if tonumber(tbl.withdraw) > 500000 then
	blckhndr_closeATM()
	TriggerEvent('blckhndr_notify:displayNotification', 'Maximum withdraw of $500,000', 'centerRight', 4000, 'error')
	return
  end
  if tonumber(tbl.withdraw) then
    if tonumber(banks) >= tonumber(tbl.withdraw) then
      ...
      TriggerEvent('blckhndr_bank:change:bankandwallet', new_wallet, new_bank)
	  TriggerServerEvent('blckhndr_main:logging:addLog', GetPlayerServerId(PlayerId()), 'money', 'Character('..exports["blckhndr_main"]:blckhndr_CharID(GetPlayerServerId(PlayerId()))..') withdrew $'..tbl.withdraw)
    else
  		TriggerEvent('blckhndr_notify:displayNotification', 'There isn\'t enough in the account!', 'centerRight', 4000, 'error')
    end
  else
    TriggerEvent('blckhndr_notify:displayNotification', 'There\'s an issue with what you entered', 'centerRight', 4000, 'error')
  end
  blckhndr_closeATM()
end)
...
RegisterNUICallback('transferMoney', function(tbl)
  ...
  if tonumber(tbl.transferAmount) > 500000 then
	blckhndr_closeATM()
	TriggerEvent('blckhndr_notify:displayNotification', 'Maximum transfer of $500,000', 'centerRight', 4000, 'error')
	return
  end
  if tonumber(tbl.transferAmount) and tonumber(tbl.transferTo) then
    if tonumber(banks) >= tonumber(tbl.transferAmount) then
      TriggerServerEvent('blckhndr_bank:transfer', tbl.transferTo, tbl.transferAmount)
    else
      TriggerEvent('blckhndr_notify:displayNotification', 'There isn\'t enough in the account!', 'centerRight', 4000, 'error')
    end
  else
