	...
	while true do Citizen.Wait(0)
		...
		if GetDistanceBetweenCoords(-1197.05078125, -893.90710449219, 13.995162963867, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1197.05078125, -893.90710449219, 13.995162963867, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				blckhndr_drawText3D(-1197.05078125, -893.90710449219, 13.995162963867, 'Burgers: '..storage.burgers..'/5\nFries: '..storage.fries..'/5\nDrinks: '..storage.drinks..'/5')
				...
				if IsControlJustPressed(0,38) then
					if not blckhndr_HasItem('ecola_drink') then
						if storage.drinks > 0 then
							TriggerEvent('blckhndr_inventory:item:add', 'ecola_drink', 1)
							storage.drinks = storage.drinks-1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'There are no drinks', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have ecola!', 'centerLeft', 4000, 'error')
					end
				end
				if IsControlJustPressed(0,23) then
					if not blckhndr_HasItem('fries') then
						if storage.fries > 0 then
							TriggerEvent('blckhndr_inventory:item:add', 'fries', 1)
							storage.fries = storage.fries-1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'There are no fries', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have fries!', 'centerLeft', 4000, 'error')
					end
				end
				if IsControlJustPressed(0,29) then
					if not blckhndr_HasItem('burger') then
						if storage.burgers > 0 then
							TriggerEvent('blckhndr_inventory:item:add', 'burger', 1)
							storage.burgers = storage.burgers-1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'There are no burger', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have burger!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
		...
		if GetDistanceBetweenCoords(-1197.462890625, -896.1552734375, 13.995164871216, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1197.462890625, -896.1552734375, 13.995164871216, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				blckhndr_drawText3D(-1197.462890625, -896.1552734375, 13.995164871216, 'Burgers: '..storage.burgers..'/5\nFries: '..storage.fries..'/5\nDrinks: '..storage.drinks..'/5')
				SetTextComponentFormat("STRING")
				AddTextComponentString("~INPUT_PICKUP~ Store eCola\n~INPUT_SPECIAL_ABILITY_SECONDARY~ Store Burger\n~INPUT_ENTER~ Store Fries")
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,38) then
					if blckhndr_HasItem('ecola_drink') then
						if storage.drinks < 5 then
							TriggerEvent('blckhndr_inventory:item:take', 'ecola_drink', 1)
							storage.drinks = storage.drinks+1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'You cannot store any more drinks '..storage.drinks..'/5', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You dont have ecola!', 'centerLeft', 4000, 'error')
					end
				end
				if IsControlJustPressed(0,23) then
					if blckhndr_HasItem('fries') then
						if storage.fries < 5 then
							TriggerEvent('blckhndr_inventory:item:take', 'fries', 1)
							storage.fries = storage.fries+1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'You cannot store any more fries '..storage.fries..'/5', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You dont have fries!', 'centerLeft', 4000, 'error')
					end
				end
				if IsControlJustPressed(0,29) then
					if blckhndr_HasItem('burger') then
						if storage.fries < 5 then
							TriggerEvent('blckhndr_inventory:item:take', 'burger', 1)
							storage.burgers = storage.burgers+1
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'You cannot store any more burgers '..storage.burgers..'/5', 'centerLeft', 4000, 'error')
						end
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You dont have burger!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
			...
			if GetDistanceBetweenCoords(-1196.7224121094, -891.4462890625, 13.995155334473, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				if IsControlJustPressed(0, 249) then
					if currentorder ~= -1 then
						...
						TriggerEvent('blckhndr_notify:displayNotification', 'Your order: '..myorder, 'centerLeft', 4000, 'info')
					end
				end
				if IsControlJustPressed(0,38) then
					if currentorder ~= -1 then
						...
						for k, item in pairs(orders[currentorder].order) do
							if not blckhndr_HasItem(item) then
								TriggerEvent('blckhndr_notify:displayNotification', 'You do not have: '..item, 'centerLeft', 4000, 'error')
								sold = false
							end
							if sold then
								if not washedhands then
									TriggerEvent('blckhndr_notify:displayNotification', 'You did not wash your hands so did not get a tip!', 'centerLeft', 4000, 'error')
								else
									local tip = math.random(1,100)
									TriggerEvent('blckhndr_notify:displayNotification', 'You got a <b>$'..tip..'</b> tip!', 'centerLeft', 4000, 'success')
									TriggerEvent('blckhndr_bank:change:walletAdd', tip)
								end
								local mini = orders[currentorder].price - 5
								local maxi = orders[currentorder].price + 10
								local cursale = math.random(mini, maxi)
								TriggerEvent('blckhndr_bank:change:walletAdd', cursale)
								TriggerEvent('blckhndr_notify:displayNotification', 'You got paid: $'..cursale, 'centerLeft', 4000, 'success')
								Citizen.Wait(500)
								currentorder = -1
								washedhands = false
							end
						...
						if currenttime >= maff then
							...
							Citizen.Wait(500)
							local myorder = table.concat(orders[currentorder].order,",")
							TriggerEvent('blckhndr_notify:displayNotification', 'You got a new order: '..myorder, 'centerLeft', 4000, 'info')
						else
							TriggerEvent('blckhndr_notify:displayNotification', 'There is no order waiting yet!', 'centerLeft', 4000, 'error')
						end
					end
				end
			end
		...
		if GetDistanceBetweenCoords(-1197.1879882813, -899.86334228516, 13.995163917542, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1197.1879882813, -899.86334228516, 13.995163917542, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,23) then
					if not blckhndr_HasItem('frozen_fries') then
						TriggerEvent('blckhndr_inventory:item:add', 'frozen_fries', 1)
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have a frozen fries!', 'centerLeft', 4000, 'error')
					end
				end
				if IsControlJustPressed(0,29) then
					if not blckhndr_HasItem('frozen_patty') then
						TriggerEvent('blckhndr_inventory:item:add', 'frozen_patty', 1)
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have a frozen fries!', 'centerLeft', 4000, 'error')
					end
				end
		...
		if GetDistanceBetweenCoords(-1197.3479003906, -901.61151123047, 13.995161056519, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1197.3479003906, -901.61151123047, 13.995161056519, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				if IsControlJustPressed(0,38) then
					if washedhands then
						TriggerEvent('blckhndr_notify:displayNotification', 'Do you have OCD? You already washed your hands', 'centerLeft', 4000, 'error')
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You washed your hands', 'centerLeft', 4000, 'info')
						washedhands = true
					end
				end
			end
			...
			if GetDistanceBetweenCoords(-1198.8013916016, -902.11535644531, 13.995160102844, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				if IsControlJustPressed(0,38) then
					...
					for k, item in pairs(required) do
						if not blckhndr_HasItem(item) then
							TriggerEvent('blckhndr_notify:displayNotification', 'You do not have: '..items_table[item].display_name, 'centerLeft', 4000, 'error')
							prepped = false
						else
							TriggerEvent('blckhndr_inventory:item:take', item, blckhndr_GetItemAmount(item))
						end
					end
					if prepped then
						TriggerEvent('blckhndr_notify:displayNotification', 'Prepping...', 'centerLeft', 1000, 'info')
						Citizen.Wait(1000)
						TriggerEvent('blckhndr_inventory:item:add', 'burger', 1)
					end
				end
		...
		if GetDistanceBetweenCoords(-1202.4093017578, -896.89923095703, 13.995160102844, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1202.4093017578, -896.89923095703, 13.995160102844, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,38) then
					if blckhndr_HasItem('frozen_patty') then
						TriggerEvent('blckhndr_inventory:item:take', 'frozen_patty', blckhndr_GetItemAmount('frozen_patty'))
						TriggerEvent('blckhndr_notify:displayNotification', 'Cooking...', 'centerLeft', 1000, 'info')
						Citizen.Wait(1000)
						TriggerEvent('blckhndr_inventory:item:add', 'cooked_patty', 1)
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You do not have a frozen patty!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
		...
		if GetDistanceBetweenCoords(-1200.6815185547, -899.34533691406, 13.995161056519, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1200.6815185547, -899.34533691406, 13.995161056519, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,38) then
					if blckhndr_HasItem('frozen_fries') then
						TriggerEvent('blckhndr_inventory:item:take', 'frozen_fries', blckhndr_GetItemAmount('frozen_fries'))
						TriggerEvent('blckhndr_notify:displayNotification', 'Cooking...', 'centerLeft', 1000, 'info')
						Citizen.Wait(1000)
						TriggerEvent('blckhndr_inventory:item:add', 'fries', 1)
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You do not have a frozen fries!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
		...
		if GetDistanceBetweenCoords(-1203.0020751953, -895.39642333984, 13.995157241821, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			...
			if GetDistanceBetweenCoords(-1203.0020751953, -895.39642333984, 13.995157241821, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,38) then
					if not blckhndr_HasItem('burger_bun') then
						TriggerEvent('blckhndr_inventory:item:add', 'burger_bun', 1)
					else
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have a burger bun!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
			...
			if GetDistanceBetweenCoords(-1199.1781005859, -894.56127929688, 13.995157241821, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				if IsControlJustPressed(0,38) then
					if not blckhndr_HasItem('ecola_drink') then
						...
						TriggerEvent('blckhndr_notify:displayNotification', 'You already have a eCola drink!', 'centerLeft', 4000, 'error')
					end
				end
			end
		end
