...
local function blckhndr_hasDirtyMoney()
  return blckhndr_HasItem('dirty_money')
end

local function blckhndr_StartLaundering()
...
local function blckhndr_NextLaundering(trig)
  if GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), true)) ~= GetHashKey("burrito") then
    TriggerEvent('blckhndr_notify:displayNotification', 'Deliveries must be completed in the vehicle provided', 'centerRight', 4000, 'error')
    return
  end
  if not trig then
    done_deliveries = done_deliveries+1
    TriggerEvent('blckhndr_notify:displayNotification', 'You completed this delivery', 'centerLeft', 4000, 'success')
  end
  if done_deliveries >= needed_delivery then
    ...
    TriggerEvent('blckhndr_notify:displayNotification', 'Head back to grove street to get your money', 'centerRight', 4000, 'info')
  else
    local new = math.random(1, #launder_deliveries)
    if new ~= current_delivery then
      RemoveBlip(delivery_blip)
      TriggerEvent('blckhndr_notify:displayNotification', 'New delivery spot added to the GPS ('.. done_deliveries+1 ..'/'..needed_delivery..')', 'centerLeft', 4000, 'info')
      ...
      blckhndr_NextLaundering(true)
    end
  end
end

local function blckhndr_FinishLaundering()
  RemoveBlip(delivery_blip)
  if blckhndr_GetItemAmount('dirty_money') == dm_amount then
    local amount = blckhndr_GetItemAmount('dirty_money')
    local minus = blckhndr_GetItemAmount('dirty_money') / quote[1]
    amount = amount - math.floor(minus)
    TriggerEvent('blckhndr_notify:displayNotification', 'Well done, here\'s your cash ($'..amount..')', 'centerLeft', 6000, 'info')
    TriggerEvent('blckhndr_bank:change:walletAdd', amount)
    TriggerEvent('blckhndr_inventory:item:take', 'dirty_money', blckhndr_GetItemAmount('dirty_money'))
  else
    TriggerEvent('blckhndr_notify:displayNotification', 'This is not the amount we agreed on!', 'centerLeft', 4000, 'error')
    ...
    --TriggerEvent('blckhndr_inventory:item:take', 'dirty_money', inventory["dirty_money"].amount)
    --TriggerEvent('blckhndr_bank:change:walletMinus', 20000)
  end
    ...
    if laundering then
      if GetDistanceBetweenCoords(launder_deliveries[current_delivery].x, launder_deliveries[current_delivery].y, launder_deliveries[current_delivery].z, GetEntityCoords(GetPlayerPed(-1)).x, GetEntityCoords(GetPlayerPed(-1)).y, GetEntityCoords(GetPlayerPed(-1)).z) < 5 then
        ...
        if IsControlJustPressed(0, 38) then
          blckhndr_NextLaundering()
        end
      end
    end
    if blckhndr_hasDirtyMoney() then
          ...
          elseif not laundering then
            if not quoted then
              ...
              if IsControlJustPressed(0, 38) then
				...
				dm_amount = blckhndr_GetItemAmount('dirty_money')
              end
            else
              SetTextComponentFormat("STRING")
              ...
              if IsControlJustPressed(0, 246) then
                ...
                blckhndr_StartLaundering(false)
              end
...
Citizen.CreateThread(function()
  while true do
	...
	if blckhndr_HasItem('dirty_money') then
		local amt = blckhndr_GetItemAmount('dirty_money')
		if amt < 5000 then
		...
		print(':blckhndr_inventory: new quote from dealer: '..quote[2])
		if not laundering then
			abletolaunder = true
		else
			print(':blckhndr_inventory: you are not able to launder as you are already laundering')
			abletolaunder = false
		end
	else
		abletolaunder = false
