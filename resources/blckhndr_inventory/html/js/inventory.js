...
function viewData(obj) {
	...
	$.post('http://blckhndr_inventory/viewData', JSON.stringify({
		inv: obj[0],
		slot: obj[1]
	}))
...
function useSlot(amt, fromInv, fromSlot) {
	...
	$.post('http://blckhndr_inventory/useSlot', JSON.stringify({
		amt: amt,
		fromInv: fromInv,
		fromSlot: fromSlot+1
	}));
...
function dropSlot(amt, fromInv, fromSlot) {
	...
	$.post('http://blckhndr_inventory/dropSlot', JSON.stringify({
		amt: amt,
		fromInv: fromInv,
		fromSlot: fromSlot+1
	}));
...
function updateSlot(amt, toInv, toSlot, fromInv, fromSlot) {
	...
	$.post('http://blckhndr_inventory/dragToSlot', JSON.stringify({
		amt: amt,
		fromInv: fromInv,
		fromSlot: fromSlot+1,
		toInv: toInv,
...
document.onkeydown = function(evt) {
    ...
    if (evt.keyCode == 27 || evt.keyCode == 112 || evt.keyCode == 9) {
        $.post('http://blckhndr_inventory/closeGUI', JSON.stringify({}))
	}
};