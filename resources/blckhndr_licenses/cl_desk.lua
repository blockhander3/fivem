...
local myChar = {}
AddEventHandler('blckhndr_main:character', function(tbl)
	myChar = tbl
end)
		...
		if GetDistanceBetweenCoords(-548.19848632813, -199.1042175293, 38.219657897949, GetEntityCoords(GetPlayerPed(-1)),true) < 50 then
			...
			if GetDistanceBetweenCoords(id_spot.x, id_spot.y, id_spot.z, GetEntityCoords(GetPlayerPed(-1)),true) < 0.5 then
				...
				if IsControlJustPressed(0,38) then
					TriggerServerEvent('blckhndr_licenses:id:request', myChar)
				end
			end
		end
	end
...
RegisterNetEvent('blckhndr_licenses:giveID')
AddEventHandler('blckhndr_licenses:giveID', function()
	TriggerServerEvent('blckhndr_licenses:id:request', myChar)
end)