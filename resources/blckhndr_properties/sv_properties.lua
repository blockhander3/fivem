...
function init()
	MySQL.Async.fetchAll('SELECT * FROM `blckhndr_properties`', {}, function(res)
		for k, p in pairs(res) do
			print(':blckhndr_properties: INIT '..p.property_name)
			if p.property_inventory and p.property_inventory ~= '' then
				...
				if tblCount(p.property_inventory) > 0 and #p.property_inventory == 0 then -- using string keys instead of numbers :thinking:
					...
					for key, val in pairs(p.property_inventory) do
						if not val.index then 
							...
							print('>> :blckhndr_properties: '..p.property_name..' has old item: '..key..' ('..val.amount..')')
							local placed = false
							for k,v in ipairs(newinv) do
								if not placed and newinv[k].index == false then
									if itemConversions[key] then
										print('>>>> :blckhndr_properties: putting item ('..key..'/'..val.amount..') in new inv slot '..k)
										newinv[k] = {}
										newinv[k] = itemConversions[key]
										newinv[k]['amt'] = val.amount
									else
										print('>>>> :blckhndr_properties: item ('..key..') does not have a conversions table')
									end
									placed = true 
								end
							end
							if not placed then
								print('>>>> :blckhndr_properties: could not find inv slot spare for item ('..key..')')
							end
						end
					end
					...
					if p.property_inventory == {} or tblCount(p.property_inventory) == 0 then
						print('>> :blckhndr_properties: '..p.property_name..' has an empty table as inv, resetting')
				...
				print('>> :blckhndr_properties: '..p.property_name..' has no inventory')
				p.property_inventory = {{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false},{index=false}}
			end
			...
			if maff < 0 then
				...
				print(':blckhndr_properties: '..sv_properties[p.property_id].name..' - RENT RAN OUT, AVAILABLE')
			end
			...
			print(':blckhndr_properties: FINISHED '..p.property_name)
			print('')
			saveProperty(p.property_id)
		end
		TriggerClientEvent('blckhndr_properties:updateXYZ', -1, sv_properties_xyz)
	end)
end
...
RegisterServerEvent('blckhndr_properties:request')
AddEventHandler('blckhndr_properties:request', function()
	TriggerClientEvent('blckhndr_properties:updateXYZ', source, sv_properties_xyz)
end)

RegisterServerEvent('blckhndr_properties:access')
AddEventHandler('blckhndr_properties:access', function(id)
	...
	if p then
		if p.inuse then
			...
			TriggerClientEvent('blckhndr_properties:access', id, sv_properties[id])
...
RegisterServerEvent('blckhndr_properties:leave')
AddEventHandler('blckhndr_properties:leave', function(id, tbl)
	local p = sv_properties[id]
	if p then
		...
		for k, v in pairs(sv_properties[id].keys) do
			if v.id and not v.name then
				local character = exports["blckhndr_main"]:blckhndr_GetCharacterInfo(v.id)
				if character then
...
RegisterServerEvent('blckhndr_properties:rent:check')
AddEventHandler('blckhndr_properties:rent:check', function(id)
	local cur_date = os.time()
	local rent_date = sv_properties[id].rent
	
	local maff = rent_date - cur_date
	maff = maff / 86400 
	TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^*^4:blckhndr_properties:^0^r Rent expires in: '..math.ceil(maff)..' days')
...
RegisterServerEvent('blckhndr_properties:rent:pay')
AddEventHandler('blckhndr_properties:rent:pay', function(id)
	local cur_date = os.time()
	local rent_date = sv_properties[id].rent
	...
	if maff < 2 then
		...
		TriggerClientEvent('blckhndr_bank:change:walletMinus', source, 25000)
...
RegisterServerEvent('blckhndr_properties:buy')
AddEventHandler('blckhndr_properties:buy', function(id)
	local p = sv_properties[id]
	if p then
		sv_properties[id].owner = exports["blckhndr_main"]:blckhndr_CharID(source)
	...
	TriggerClientEvent('blckhndr_properties:updateXYZ', -1, sv_properties_xyz)
		...
		TriggerClientEvent('blckhndr_bank:change:walletMinus', source, 50000)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Success. Rent will be due in 7 days time.' })
	else
...
function saveProperty(id)
	...
	MySQL.Sync.execute("UPDATE `blckhndr_properties` SET `property_owner` = @owner, `property_coowners` = @keys, `property_inventory` = @inv, `property_weapons` = @weps, `property_money` = @money, `property_expiry` = @rent WHERE `property_id` = @id;",  {
		['@id'] = id,
		['@owner'] = owner,
		['@keys'] = keys,
		['@inv'] = inv,
...
local realators = {1}
RegisterServerEvent('blckhndr_properties:realator:clock')
AddEventHandler('blckhndr_properties:realator:clock', function()
	
end)
function blckhndr_isRealator(id)
