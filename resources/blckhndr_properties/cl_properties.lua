...
AddEventHandler('blckhndr_main:character', function(c)
	-- what properties do i have keys to papa
	TriggerServerEvent('blckhndr_properties:request')
	TriggerServerEvent('blckhndr_properties:requestKeys', c.char_id)
end)

RegisterNetEvent('blckhndr_properties:updateXYZ')
AddEventHandler('blckhndr_properties:updateXYZ', function(tbl)
			...
			if GetDistanceBetweenCoords(p.xyz.x,p.xyz.y,p.xyz.z,GetEntityCoords(GetPlayerPed(-1)),true) < 20 then
				...
				if GetDistanceBetweenCoords(p.xyz.x,p.xyz.y,p.xyz.z,GetEntityCoords(GetPlayerPed(-1)),true) < 0.5 then
					...
					if IsControlJustPressed(0,38) then
						TriggerServerEvent('blckhndr_properties:access', k)
					end
				end
			end
		end
...
RegisterNetEvent('blckhndr_properties:access')
AddEventHandler('blckhndr_properties:access', function(id, p)
	current_p_id = id
	p = sv_properties[id]
	current_p = p

	
	if current_p.owner == exports["blckhndr_main"]:blckhndr_CharID() then
	...
	for k, v in pairs(current_p.keys) do
		if v.id == exports["blckhndr_main"]:blckhndr_CharID() then
			current_p_Owner = false
			current_p_Keys = true
		end
	end
	if keez then hasKeys = true else hasKeys = false end
	if exports["blckhndr_police"]:blckhndr_getPDLevel() > 5 then isPoPoCMD = false else isPoPoCMD = false end
	ToggleActionMenu()
end)

RegisterNetEvent('blckhndr_properties:inv:update')
AddEventHandler('blckhndr_properties:inv:update', function(inv)
...
function depositWeapon()
	...
	if submitting == false then
		...
		local info = exports["blckhndr_criminalmisc"]:weaponInfo(GetSelectedPedWeapon(GetPlayerPed(-1)))
		if info.name then
...
function takeWeapon(id)
	...
	if wep then
		if wep.name then
			...
			TriggerEvent('blckhndr_criminalmisc:weapons:add:tbl', wep)
...
function checkRent()
	TriggerServerEvent('blckhndr_properties:rent:check', current_p_id)
end
        ...
        if UpdateOnscreenKeyboard() ~= 0 then
          ...
          if UpdateOnscreenKeyboard() == 1 then
			...
			if amount <= 100000 and amount > 0 then
				if exports["blckhndr_main"]:blckhndr_CanAfford(amount) then
					current_p.money = current_p.money + amount
					TriggerEvent('blckhndr_bank:change:walletMinus', amount)
          ...
          if UpdateOnscreenKeyboard() == 1 then
			...
			if amount <= 100000 and amount > 0 then
				if current_p.money >= amount then
					current_p.money = current_p.money - amount
					TriggerEvent('blckhndr_bank:change:walletAdd', amount)
...
RegisterNetEvent('blckhndr_properties:inv:closed')
AddEventHandler('blckhndr_properties:inv:closed', function(id)
	Citizen.Wait(100)
	ToggleActionMenu()
end)
...
function ToggleActionMenu(send)
	...
	if ( menuEnabled ) then
		...
		SetNuiFocus( false )
		if send then
			TriggerServerEvent('blckhndr_properties:leave', current_p_id, current_p)
			current_p_id = 0
			current_p = {}
		end
		SendNUIMessage({
...
RegisterNUICallback( "ButtonClick", function( data, cb )
	...
	elseif split[1] == 'info' then
		if split[2] == 'show' then
			TriggerEvent('chatMessage', '', {255,255,255}, '^*^4:blckhndr_properties:^0^r '..current_p.name..' | '..current_p.owner)
			checkRent()
		elseif split[2] == 'rent' then
			if exports["blckhndr_main"]:blckhndr_CanAfford(25000) then
				TriggerServerEvent('blckhndr_properties:rent:pay', current_p_id)
			else
				exports['mythic_notify']:DoHudText('error', 'You cannot afford this. ($25,000)')
			end
		elseif split[2] == 'buy' then
			if exports["blckhndr_main"]:blckhndr_CanAfford(50000) then
				TriggerServerEvent('blckhndr_properties:buy', current_p_id)
				ToggleActionMenu(true)
			else
	...
	elseif split[1] == 'inventory' then
		...
		ToggleActionMenu(false)
		TriggerEvent('blckhndr_inventory:prop:recieve', current_p_id, current_p.inventory)
