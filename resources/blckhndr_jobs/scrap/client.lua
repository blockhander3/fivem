      ...
      if IsPedInAnyVehicle(GetPlayerPed(-1)) then
        ...
        if GetEntityModel(car) == GetHashKey(required) then
          ...
          if IsControlJustPressed(0,38) then
			if exports["blckhndr_cargarage"]:blckhndr_IsVehicleOwner(car) then
				...
				required = false
				lastjob = 0
				TriggerEvent('blckhndr_inventory:item:add', 'dirty_money', math.random(5000,12000))
          ...
          SetTextComponentFormat("STRING")
          AddTextComponentString("~r~Ez nem a jó jármű..")
          DisplayHelpTextFromStringLabel(0, 0, 1, -1)
        end
      end
    end
    if GetDistanceBetweenCoords(laptop.x, laptop.y, laptop.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
      ...
      if GetDistanceBetweenCoords(laptop.x, laptop.y, laptop.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
        if required == false then
          SetTextComponentFormat("STRING")
          AddTextComponentString("Nyomj ~INPUT_PICKUP~ gombot, hogy munkát kérj.")
          DisplayHelpTextFromStringLabel(0, 0, 1, -1)
          if IsControlJustPressed(0,38) then
            required = cars[math.random(1, #cars)]
            lastjob = GetNetworkTime()
            TriggerEvent('chatMessage', '', {255,255,255}, '^5^*DARK WEB |^0^r Keress nekünk egy ilyen járművet: '..GetDisplayNameFromVehicleModel(required))
          end
        else
          if lastjob + 20 > GetNetworkTime() then
            ...
            DisplayHelpTextFromStringLabel(0, 0, 1, -1)
            if IsControlJustPressed(0,38) then
              local vehs = exports["blckhndr_entfinder"]:getVehicles(false)
            ...
            AddTextComponentString('~r~Ilyen jáművet keresel: ~w~'..required);
