...
local function blckhndr_taxiAddBlip()
...
local disp_id = 0
local last_disp = 0
RegisterNetEvent('blckhndr_jobs:taxi:request')
AddEventHandler('blckhndr_jobs:taxi:request', function(tbl)
  if istaxi then
    local x = tbl.x
    local y = tbl.y
    ...
    AddTextComponentString('Helyszín: ~y~'..sname);
    SetNotificationMessage("CHAR_DEFAULT", "CHAR_DEFAULT", true, 1, "Taxi Dispatch", "");
    DrawNotification(false, true);
  end
end)
RegisterNetEvent('blckhndr_jobs:taxi:accepted')
AddEventHandler('blckhndr_jobs:taxi:accepted', function(taxi)
  yourtaxi = taxi
end)
   ...
   while true do
     ...
     if disp_id ~= 0 then
       if last_disp + 10 > current_time then
         ...
         AddTextComponentString("Nyomj ~INPUT_MP_TEXT_CHAT_TEAM~ gombot, hogy ~g~elfogadd~w~ a hívást\nNyomj ~INPUT_PUSH_TO_TALK~ gombot, hogy ~r~elutasítsd~w~ a hívást")
      ...
      if GetDistanceBetweenCoords(v.x,v.y,v.z,GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
        ...
        if GetDistanceBetweenCoords(v.x,v.y,v.z,GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
          if istaxi == false then
            SetTextComponentFormat("STRING")
            AddTextComponentString("Nyomj ~INPUT_PICKUP~ gombot, hogy lekérj egy taxit (~g~$150~w~)")
          	DisplayHelpTextFromStringLabel(0, 0, 1, -1)
            if IsControlJustPressed(0,38) then
              ...
              TriggerEvent('blckhndr_notify:displayNotification', 'Vidd el a taxid a parkolóból!', 'centerRight', 4000, 'info')
            	TriggerEvent('blckhndr_cargarage:makeMine', mytaxi, GetDisplayNameFromVehicleModel(GetEntityModel(mytaxi)), GetVehicleNumberPlateText(mytaxi))
              istaxi = true
              blckhndr_SetJob('Taxi Driver')
              TriggerEvent('blckhndr_bank:change:walletMinus', 150)
            end
          else
            SetTextComponentFormat("STRING")
            AddTextComponentString("Nyomj ~INPUT_PICKUP~ gombot, hogy ~r~visszaadd~w~ a taxidat (~g~+$150~w~)")
          	DisplayHelpTextFromStringLabel(0, 0, 1, -1)
            if IsControlJustPressed(0,38) then
              if DoesEntityExist(mytaxi) then
                ...
                TriggerEvent('blckhndr_notify:displayNotification', 'A taxid jó állapotban került vissza', 'centerRight', 4000, 'info')
                TriggerEvent('blckhndr_bank:change:bankAdd', 150)
                istaxi = false
                mytaxi = false
                blckhndr_SetJob('Unemployed')
              else
                TriggerEvent('blckhndr_notify:displayNotification', 'Összetörted a taxidat! Ára: $2500!', 'centerRight', 4000, 'error')
                TriggerEvent('blckhndr_bank:change:bankMinus', 2500)
                istaxi = false
                mytaxi = false
                blckhndr_SetJob('Unemployed')
              end
...
function StartJob(jobid)
	if jobid == 1 then -- taxi
		...
		Wait(500)
		DrawMissionText("Menj, és keress a környéken ~h~<font color='#f6fc46'>utasokat~w~.", 10000)
		onJob = jobid
	end
end
...
Citizen.CreateThread(function()
	while true do
		Wait(5)
		if onJob == 0 then
			if blckhndr_GetJob() == 'Taxi Driver' then -- DEL
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							DrawMissionText("Az utasod  <font color='#ea002e'>meghalt</font>. Keress másikat.", 5000)
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("Az utas <font color='#ea002e'>nem vár tovább</font>. Keress másikat.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("<font color='#ea002e'>Az utas nem akar veled menni</font>. Keress másikat.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("Vigyél el ide: %s, közel: %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
											DrawMissionText(streetname, 5000)
										else
											local streetname = string.format("Vigyél el ide: %s", GetStreetNameFromHashKey(street[1]))
											DrawMissionText(streetname, 5000)
										end
							...
							if jobs.flag[1] == 3 then
								if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]], true) > 4.0001 then
									...
									Wait(6000)
									DrawMissionText("<font color='#00ea1b'>Sikeresen elvitted az utasodat! Távolság:" .. (distance / 30) .. " Tip: " .. tip, 4000)

									local payment = math.ceil((distance / 25) + tip)
									TriggerEvent("blckhndr_bank:change:walletAdd", payment)
									Wait(5000)
									DrawMissionText("Menj, és keress a közelben ~h~<font color='#f6fc46'>utasokat~w~.", 10000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								end
							end
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									DrawMissionText("<font color='#00ea1b'>Az utas~w~ rád vár. Menj közel hozzá", 5000)
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 50.0001 then
						...
						DrawMissionText("Szállj vissza a kocsidba, hogy folytathasd a munkát. Vagy menj messzebb, es fejezd be a munkád.", 1)
					end
				end
			else
				StopJob(1)
				DrawMissionText("A taxi ~h~<font color='#ea002e'>leadva~w~.", 5000)
			end
		end
	end
end)
