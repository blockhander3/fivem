...
local availableTows = {}
RegisterNetEvent('blckhndr_jobs:tow:marked')
AddEventHandler('blckhndr_jobs:tow:marked', function(plate, officer)
...
function SpawnTowTruck(x, y, z)
  ...
  if canAfford then
    ...
    TriggerEvent('blckhndr_fuel:update', GetVehicleNumberPlateText(towtruck), 100)

    TriggerEvent('blckhndr_cargarage:makeMine', towtruck, GetDisplayNameFromVehicleModel(GetEntityModel(towtruck)), GetVehicleNumberPlateText(towtruck))
    inService = true
  end
end
...
function ImpoundVehicle()
  ...
  if towedvehicle then
    ...
    if DecorGetInt(towedvehicle, 'jTow:marked') == 1 then
      TriggerEvent('blckhndr_bank:change:walletAdd', math.random(1000, 1300))
    else
      TriggerEvent('blckhndr_bank:change:walletAdd', math.random(100, 500))
    end
...
function ReturnTruck()
  ...
  TriggerEvent('blckhndr_bank:change:walletAdd', 1500)
  towtruck = false
  inService = false
end
...
local disp_id = 0
local last_disp = 0
RegisterNetEvent('blckhndr_jobs:tow:request')
AddEventHandler('blckhndr_jobs:tow:request', function(tbl)
  if inService then
    local x = tbl.x
    local y = tbl.y
	...
	TriggerEvent("blckhndr_main:blip:add", "tow", "ALERT: Tow Request", 317, x, y, 0.0)
      ...
      if GetDistanceBetweenCoords(v.centerPoint.x,v.centerPoint.y,v.centerPoint.z,GetEntityCoords(GetPlayerPed(-1))) < 40 then
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1), true) == false and GetDistanceBetweenCoords(v.centerPoint.x,v.centerPoint.y,v.centerPoint.x,GetEntityCoords(GetPlayerPed(-1))) < 4 then
          if towtruck then
            ...
            if IsControlJustPressed(1,51) then
              if exports.blckhndr_main:blckhndr_GetWallet() >= 1500 then
                TriggerEvent('blckhndr_bank:change:walletMinus', 1500)
