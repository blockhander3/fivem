local errprefix = "^3[blckhndr_builders]^7 ^1ERROR:^7 "
--[[
	Translators
]]
...
function builder.shouldBuild(res)
	return GetNumResourceMetadata(res, "blckhndr_handling") > 0
end
...
function builder.build(res, finished)
	...
	for i=1, GetNumResourceMetadata(res, "blckhndr_handling") do
		local file = GetResourceMetadata(res, "blckhndr_handling", i-1)
		xml:comment(file)
...
RegisterResourceBuildTaskFactory("blckhndr_handling", factory)