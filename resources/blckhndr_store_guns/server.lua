...
RegisterServerEvent('blckhndr_store_guns:request')
AddEventHandler('blckhndr_store_guns:request', function(store_id)
		...
		if s.busy == false then
			...
			for k, v in pairs(s.stock) do
				if items[k] then
					...
					print('ERROR (blckhndr_store_guns) :: Item '..k..' is not defined in server.lua')
				end
			end

			TriggerClientEvent('blckhndr_inventory:store_gun:recieve', source, store_id, inv)
		else
...
RegisterServerEvent('blckhndr_store_guns:boughtOne')
AddEventHandler('blckhndr_store_guns:boughtOne', function(store_id, item)
	local s = stores[store_id]
	if s then
...
RegisterServerEvent('blckhndr_store_guns:closedStore')
AddEventHandler('blckhndr_store_guns:closedStore', function(store_id)
	local s = stores[store_id]
	if s then
		s.busy = false
	else 
