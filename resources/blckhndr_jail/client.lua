...
RegisterNetEvent('blckhndr_jail:spawn:recieve')
AddEventHandler('blckhndr_jail:spawn:recieve', function(result)
	jailtime = result
  if jailtime >= 1 then
    TriggerEvent('blckhndr_jail:sendme', jailtime)
    TriggerServerEvent('blckhndr_jail:update:database', jailtime)
  else
    jailtime = 0
    beginSentence = 0
    isinjail = false
...
RegisterNetEvent('blckhndr_jail:sendme')
AddEventHandler('blckhndr_jail:sendme', function(time)
	jailtime = time
	totaltime = time
  isinjail = true
  ...
  TriggerEvent('pNotify:SendNotification', {text = "Be lettél börtönözve: ".. math.floor(tonumber(jailtime) / 60) .." percre",
    layout = "centerRight",
    timeout = 600,
    progressBar = true,
    type = "info"
  })
  TriggerEvent('blckhndr_hungerandthirst:pause')
end)

RegisterNetEvent('blckhndr_jail:releaseme')
AddEventHandler('blckhndr_jail:releaseme', function()
	jailtime = 0
  isinjail = false
  SetEntityCoords(GetPlayerPed(-1), 1852.42, 2603.44,45.672)
  TriggerServerEvent('blckhndr_jail:update:database', jailtime)
  TriggerEvent('pNotify:SendNotification', {text = "Ki lettél engedvea börtönből",
    layout = "centerRight",
    timeout = 600,
    progressBar = true,
    type = "info"
  })
  TriggerEvent('blckhndr_hungerandthirst:unpause')
end)

RegisterNetEvent('blckhndr_jail:init')
AddEventHandler('blckhndr_jail:init', function(char_id)
  characterid = char_id
  TriggerServerEvent("blckhndr_jail:spawn", characterid)
end)
    ...
    if isinjail then
		...
		if maff <= timetick then
			if myjob == 0 then
				if jailtime > totaltime / 50 then
					...
					lastjob = timetick
					TriggerEvent('blckhndr_notify:displayNotification', 'Kaptál egy új munkát (#'..myjob..'), menj oda, hogy lerövidítsd a büntetésed!', 'centerRight', 4000, 'info')
				end
			end
		else
		...
		if myjob ~= 0 then
			...
			if GetDistanceBetweenCoords(job.x, job.y, job.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				lastjob = timetick
				myjob = 0
				TriggerEvent('blckhndr_notify:displayNotification', 'Szép munka! '..sex..' másodperccel rövidítve a bűntetés!', 'centerRight', 4000, 'success')
			end
		end
      local inmins = tonumber(jailtime) / 60
      drawTxt(1.24, 1.44, 1.0,1.0,0.4, "Börtön: ~r~" .. math.floor(inmins) .. "~w~ perc van hátra", 255, 255, 255, 255)
    end
  end
end)
  ...
  while true do
    ...
    if isinjail then
	  ...
	  if GetDistanceBetweenCoords(1691.8676757813, 2606.4614257813, 45.560329437256,GetEntityCoords(GetPlayerPed(-1)), true) > 330 then
		...
		TriggerEvent('blckhndr_notify:displayNotification', 'A bűntetésedet a börtönben kell letoltened!', 'centerRight', 6000, 'error')
	  end
      if jailtime <= 0 then
        TriggerEvent('blckhndr_jail:releaseme')
      end
    end
  end
end)
