...
RegisterNetEvent('blckhndr_commands:police:lock')
AddEventHandler('blckhndr_commands:police:lock', function()
	if GetDistanceBetweenCoords(doors[1].x, doors[1].y, doors[1].z, GetEntityCoords(GetPlayerPed(-1)), true) < 3 or GetDistanceBetweenCoords(doors[2].x, doors[2].y, doors[2].z, GetEntityCoords(GetPlayerPed(-1)), true) < 3 then
		...
		TriggerServerEvent('blckhndr_jewellerystore:doors:toggle')
	end
end)
RegisterNetEvent('blckhndr_jewellerystore:doors:State')
AddEventHandler('blckhndr_jewellerystore:doors:State', function(state)
	amilocked = state
end)
...
RegisterNetEvent('blckhndr_jewellerystore:gasDoor:toggle')
AddEventHandler('blckhndr_jewellerystore:gasDoor:toggle', function()
...
RegisterNetEvent('blckhndr_jewellerystore:cases:update')
AddEventHandler('blckhndr_jewellerystore:cases:update', function(tbl)
	cases = tbl
end)

RegisterNetEvent('blckhndr_jewellerystore:case:startrob')
AddEventHandler('blckhndr_jewellerystore:case:startrob', function(caseid)
	...
	robbing = false
	TriggerEvent('blckhndr_inventory:item:add', 'dirty_money', math.random(400,1000))
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(outside.x, outside.y, outside.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
			if not exports["blckhndr_police"]:blckhndr_PDDuty() and IsPedDriving() then
				local pos = GetEntityCoords(GetPlayerPed(-1))
				local coords = {
					x = pos.x,
					y = pos.y,
				...
				TriggerServerEvent('blckhndr_police:dispatch', coords, 13, '10-37 (Gyanús jármü) | Jármű: '..vehicle..' | Rendszám: '..plate..' | Szín: '..colour)
				Citizen.Wait(10000)
			end
		end
	end
	...
	while true do
		...
		if GetDistanceBetweenCoords(doors[1].x, doors[1].y, doors[1].z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 or GetDistanceBetweenCoords(doors[2].x, doors[2].y, doors[2].z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
			if amilocked then
				for k, v in pairs(doors) do
					...
					blckhndr_drawText3D(v.x,v.y,v.z, '~r~ZÁRVA')
		...
		if GetDistanceBetweenCoords(gasdoor.loc.x,gasdoor.loc.y,gasdoor.loc.z, GetEntityCoords(GetPlayerPed(-1)), true) < 5 then
			if isgasdoorLocked then
				blckhndr_drawText3D(gasdoor.txt.x,gasdoor.txt.y,gasdoor.txt.z, '~r~ZÁRVA\n~w~[E] KÁRTYA LEOLVASÁSA')
				local door = GetClosestObjectOfType(gasdoor.loc.x,gasdoor.loc.y,gasdoor.loc.z, 1.0, gasdoor.mdl, false, false, false)
				FreezeEntityPosition(door, true)
				if IsControlJustPressed(0,38) then
					if exports["blckhndr_inventory"]:blckhndr_HasItem('keycard') then
						TriggerServerEvent('blckhndr_inventory:gasDoorunlock')
					end
				end
			else
				local door = GetClosestObjectOfType(gasdoor.loc.x,gasdoor.loc.y,gasdoor.loc.z, 1.0, gasdoor.mdl, false, false, false)
		...
		if GetDistanceBetweenCoords(3563.146484375, 3673.47265625, 28.121885299683, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
			...
			if GetDistanceBetweenCoords(3563.146484375, 3673.47265625, 28.121885299683, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				...
				AddTextComponentString("TIPP: Hasnálj 'Üres kannát'")
		...
		if GetDistanceBetweenCoords(1660.0181884766, 6.2703385353088, 166.11819458008, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
			...
			if GetDistanceBetweenCoords(1660.0181884766, 6.2703385353088, 166.11819458008, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
				SetTextComponentFormat("STRING")
				AddTextComponentString("~INPUT_PICKUP~ kanna felvétele")
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0,38) then
					TriggerEvent('blckhndr_inventory:item:add', 'empty_canister', 1)
				end
			end
		end
		...
		if GetDistanceBetweenCoords(centre.x, centre.y, centre.z, GetEntityCoords(GetPlayerPed(-1)), true) < 30 then
			...
			--	blckhndr_drawText3D(-622.29168701172, -229.90211486816, 38.057048797607, '[E] ~g~Purchase:~w~ Ring\n~y~($100,000)')
			--end
			-------- NEW GUARD SHIT
			for key, guard in pairs(guardlocs) do
				if guard.ped == false then
					if exports["blckhndr_entfinder"]:getPedNearCoords(guard.x, guard.y, guard.z,1) then
						local found = false
						for k,v in ipairs(guardmdls) do
							if GetEntityModel(exports["blckhndr_entfinder"]:getPedNearCoords(guard.x, guard.y, guard.z,2)) == GetHashKey(v) then
								found = true
							end
						end
						if found then
							guard.ped = exports["blckhndr_entfinder"]:getPedNearCoords(guard.x, guard.y, guard.z,2)
						else
			...
			if not blips then
				for key, case in pairs(cases) do
					...
					AddTextComponentString("Rabolható szekrény")
			...
			for key, case in pairs(cases) do
				...
				if GetDistanceBetweenCoords(case[1],case[2],case[3], GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
					...
					if robbing then
						...
						blckhndr_drawText3D(GetEntityCoords(GetPlayerPed(-1)).x,GetEntityCoords(GetPlayerPed(-1)).y,GetEntityCoords(GetPlayerPed(-1)).z, 'Looting...')
						FreezeEntityPosition(GetPlayerPed(-1), true)
					elseif case.robbed == false then 
						if exports["blckhndr_police"]:blckhndr_getCopAmt() > 2 then
							blckhndr_drawText3D(case[1],case[2],case[3], '[E] Szekrény átkutatása')
							if IsControlJustPressed(0,38) then
								-- e is pressed
								TriggerServerEvent('blckhndr_jewellerystore:case:rob', key)
								TriggerGuardAttack()
							end
						else
							blckhndr_drawText3D(case[1],case[2],case[3], '~r~Nincs senki sem, aki megállitáhatna')
						end
					else
						blckhndr_drawText3D(case[1],case[2],case[3], 'Már átkutatott szekrény')
					end
				else
					--if robbing then
					--	print'robbing'
