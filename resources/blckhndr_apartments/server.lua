...
RegisterServerEvent('blckhndr_apartments:getApartment')
AddEventHandler('blckhndr_apartments:getApartment', function(char_id)
  local source = source
	local appt = MySQL.Sync.fetchAll("SELECT * FROM `blckhndr_apartments` WHERE `apt_owner` = '"..char_id.."'")
	if #appt > 0 then
		local myappt = appt[1]
		...
		TriggerClientEvent('blckhndr_apartments:sendApartment', source, sendappt)
	else
		TriggerClientEvent('blckhndr_apartments:characterCreation', source)
	end
end)
...
RegisterServerEvent('blckhndr_apartments:createApartment')
AddEventHandler('blckhndr_apartments:createApartment', function(char_id)
	local source = source
	--Citizen.CreateThread(function()
		print('creating new appt for '..char_id)
		local SQL = "INSERT INTO `blckhndr_apartments` (`apt_id`, `apt_owner`, `apt_inventory`, `apt_cash`, `apt_outfits`, `apt_utils`) VALUES (NULL, '"..char_id.."', '{}', '0', '{}', '{}');"
...
RegisterServerEvent('blckhndr_apartments:saveApartment')
AddEventHandler('blckhndr_apartments:saveApartment', function(appt)
--UPDATE `blckhndr_datastore`.`blckhndr_apartments` SET `apt_inventory` = '[]', `apt_cash` = '100', `apt_outfits` = '[]', `apt_utils` = '[]' WHERE `blckhndr_apartments`.`apt_id` = 4;
	--local SQL = "UPDATE blckhndr_apartments SET `apt_inventory` = '"..json.encode(appt.apt_inventory).."', `apt_cash` = '"..appt.apt_cash.."', `apt_outfits` = '"..json.encode(appt.apt_outfits).."', apt_utils` = '"..json.encode(appt.apt_utils).."' WHERE `apt_id` = '"..appt.apt_id.."'"
	MySQL.Sync.execute("UPDATE `blckhndr_apartments` SET `apt_inventory` = @inv, `apt_cash` = @cash, `apt_outfits` = @outfits, `apt_utils` = @utils WHERE `blckhndr_apartments`.`apt_id` = @id;", {
		['@id'] = appt.apt_id,
...
AddEventHandler('chatMessage', function(source, auth, msg)
	...
	if split[1] == '/stash' then
		if split[2] == 'add' then
			if tonumber(split[3]) then
				TriggerClientEvent('blckhndr_apartments:stash:add', source, tonumber(split[3]))
			else
		...
		if split[2] == 'take' then
			if tonumber(split[3]) then
				TriggerClientEvent('blckhndr_apartments:stash:take', source, tonumber(split[3]))
			else
				TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^1^*:FSN:^0^r That is not a number.')
			end
		end
	...
	if split[1] == '/outfit' then
		if split[2] == 'add' then
			if split[3] then
				TriggerClientEvent('blckhndr_apartments:outfit:add', source, split[3])
			end
		end
		if split[2] == 'use' then
			if split[3] then
				TriggerClientEvent('blckhndr_apartments:outfit:use', source, split[3])
			end
		end 
		if split[2] == 'remove' then 
			TriggerClientEvent('blckhndr_apartments:outfit:remove', source, split[3])
		end
		if split[2] == 'list' then 
			TriggerClientEvent('blckhndr_apartments:outfit:list', source)
		end
	end
end)