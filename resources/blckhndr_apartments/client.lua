...
RegisterNetEvent('blckhndr_apartments:stash:add')
AddEventHandler('blckhndr_apartments:stash:add', function(amt)
	if inappt then
		if amt > 0 and amt < 150000 then
			if exports["blckhndr_main"]:blckhndr_CanAfford(amt) then
				if apptdetails.apt_cash + amt <= 150000 then
					apptdetails.apt_cash = apptdetails.apt_cash + amt
					TriggerEvent('blckhndr_bank:change:walletMinus', amt)
					TriggerServerEvent('blckhndr_apartments:saveApartment', apptdetails)
				else
					TriggerEvent('blckhndr_notify:displayNotification', 'You cannot store this amount.', 'centerRight', 4000, 'error')
				end
			else
				TriggerEvent('blckhndr_notify:displayNotification', 'You cannot afford this.', 'centerRight', 4000, 'error')
			end	
		end
	else
		TriggerEvent('blckhndr_notify:displayNotification', 'You need to be in your appartment.', 'centerRight', 4000, 'error')
	end
end)

RegisterNetEvent('blckhndr_apartments:stash:take')
AddEventHandler('blckhndr_apartments:stash:take', function(amt)
	if inappt then
		if amt > 0 and amt < 150000 then
			if apptdetails.apt_cash >= amt then
				apptdetails.apt_cash = apptdetails.apt_cash - amt
				TriggerEvent('blckhndr_bank:change:walletAdd', amt)
				TriggerServerEvent('blckhndr_apartments:saveApartment', apptdetails)
			else
				TriggerEvent('blckhndr_notify:displayNotification', 'Your stash does not have that much.', 'centerRight', 4000, 'error')
			end
		end
	else
		TriggerEvent('blckhndr_notify:displayNotification', 'You need to be in your appartment.', 'centerRight', 4000, 'error')
...
RegisterNetEvent('blckhndr_apartments:sendApartment')
AddEventHandler('blckhndr_apartments:sendApartment', function(tbl)
	if tbl.number then
		...
		elseif apptdetails["apt_utils"]["inventory"].newinv ~= true then
			...
			for k,v in pairs(apptdetails["apt_utils"]["inventory"]) do
				...
				local item = exports['blckhndr_inventory']:blckhndr_GetItemDetails(k)
				if item then
					item.amt = v
		...
		init = true
		TriggerEvent('blckhndr_notify:displayNotification', 'Your apartment number is: '..tbl.number, 'centerRight', 6000, 'info')
	else
		...
		init = true
		TriggerEvent('blckhndr_notify:displayNotification', 'Your apartments is number: 1', 'centerRight', 6000, 'info')
		TriggerEvent('chatMessage', '', {255,255,255}, '^1^*:FSN:^0^r You can still play, but this apartment will not save.')
	end
end)

RegisterNetEvent('blckhndr_apartments:outfit:add')
AddEventHandler('blckhndr_apartments:outfit:add', function(key)
	--if inWardrobe then
		apptdetails.apt_outfits[key] = exports["blckhndr_clothing"]:GetOutfit()
...
RegisterNetEvent('blckhndr_apartments:outfit:use')
AddEventHandler('blckhndr_apartments:outfit:use', function(key)
	--if inWardrobe then
...
RegisterNetEvent('blckhndr_apartments:outfit:remove')
AddEventHandler('blckhndr_apartments:outfit:remove', function(key)
	--if inWardrobe then
		if apptdetails.apt_outfits[key] then
...
RegisterNetEvent('blckhndr_apartments:outfit:list')
AddEventHandler('blckhndr_apartments:outfit:list', function()
	--if inWardrobe then
...
apartments = {
	[1] = {x = -166.90464782715, y = 6439.720703125, z = 31.916030883789},
	[2] = {x = -166.90464782715, y = 6439.720703125, z = 31.916030883789},
	[3] = {x = -149.70074462891, y = 6422.5366210938, z = 31.916032791138}, 
	[4] = {x = -149.86361694336, y = 6416.1860351562, z = 31.916032791138}, 
	[5] = {x = -156.83972167969, y = 6409.2182617188, z = 31.916175842285},
...
local outfitsmark = {
	{x = 1849.7237548828, y = 3695.5393066406, z = 34.262008666992},
	{x = -456.84579467773, y = 6014.3266601562, z = 31.71654510498}
}
...
RegisterNetEvent('blckhndr_apartments:inv:update')
AddEventHandler('blckhndr_apartments:inv:update', function(tbl)
...
RegisterNUICallback( "weaponEquip", function( data, cb )
	...
	for key, wep in pairs(apptdetails["apt_utils"]["weapons"]) do
		if wep.name == data.name then
			TriggerEvent('blckhndr_criminalmisc:weapons:add:tbl', apptdetails["apt_utils"]["weapons"][key])
			TriggerEvent('blckhndr_notify:displayNotification', 'You equipped '..wep.name, 'centerRight', 6000, 'success')
...
RegisterNUICallback( "ButtonClick", function( data, cb )
	...
	if data == 'weapon-putaway' then
		...
		if GetSelectedPedWeapon(GetPlayerPed(-1)) == -1569615261 then
			TriggerEvent('blckhndr_notify:displayNotification', 'You cannot store your fists.', 'centerRight', 10000, 'error')
			return
		end
		local weapon = exports["blckhndr_criminalmisc"]:weaponInfo(GetSelectedPedWeapon(GetPlayerPed(-1)))
		print(json.encode(weapon))
		table.insert(apptdetails["apt_utils"]["weapons"],#apptdetails["apt_utils"]["weapons"]+1, weapon)
		TriggerEvent('blckhndr_criminalmisc:weapons:destroy')
		saveApartment()
		ExecuteCommand('save')
	elseif data == 'inventory' then
		-- open the inventory
		ToggleActionMenu()
		TriggerEvent('blckhndr_inventory:apt:recieve', myRoomNumber, apptdetails["apt_utils"]["inventory"]["table"])
		saveApartment()
		ExecuteCommand('save')
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if init then
			for k, outfitsmark in pairs(outfitsmark) do
			DrawMarker(25, outfitsmark.x, outfitsmark.y, outfitsmark.z, - 1.55, 0, 0, 0, 0, 0, 0, 0.50, 0.50, 10.3, 255, 255, 255, 140, 0, 0, 1, 0, 0, 0, 0)
				if GetDistanceBetweenCoords(outfitsmark.x, outfitsmark.y, outfitsmark.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1.5 then
					blckhndr_drawText3D(outfitsmark.x, outfitsmark.y, outfitsmark.z, "/outfit add {név}\n/outfit use {név}\n/outfit remove {név}\n/outfit list")	
				end
			end
			if inappt then
				-- storage
				...
				if GetDistanceBetweenCoords(storage.x, storage.y, storage.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
					blckhndr_drawText3D(storage.x, storage.y, storage.z, "[E] access storage")--\n~r~Not available yet")
				...
				if GetDistanceBetweenCoords(cash.x, cash.y, cash.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
					blckhndr_drawText3D(cash.x, cash.y, cash.z, "$"..apptdetails.apt_cash.." / $150,000\n\n/stash add {amt}\n/stash take {amt}")
				end
				...
				if GetDistanceBetweenCoords(outfits.x, outfits.y, outfits.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
					blckhndr_drawText3D(outfits.x, outfits.y, outfits.z, "/outfit add {name}\n/outfit use {name}\n/outfit remove {name}\n/outfit list")
					inWardrobe = true
				else
					inWardrobe = false
				end
				...
				if GetDistanceBetweenCoords(leave.x, leave.y, leave.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
					blckhndr_drawText3D(leave.x, leave.y, leave.z, "[E] Leave Apartment")
					if IsControlJustPressed(0,38) then
						...
						--instanceMe(false)
						TriggerServerEvent('blckhndr_apartments:instance:leave')
					end
				end
				...
				if GetDistanceBetweenCoords(apartments[myRoomNumber].x, apartments[myRoomNumber].y, apartments[myRoomNumber].z, GetEntityCoords(GetPlayerPed(-1))) < 20 then
					...
					if GetDistanceBetweenCoords(apartments[myRoomNumber].x, apartments[myRoomNumber].y, apartments[myRoomNumber].z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
						blckhndr_drawText3D(apartments[myRoomNumber].x, apartments[myRoomNumber].y, apartments[myRoomNumber].z+1, "[E] Enter Apartment")
						if IsControlJustPressed(0,38) then
							EnterRoom(myRoomNumber)
						end
					end
...
function EnterRoom(id)
	...
	--instanceMe(true)
	TriggerServerEvent('blckhndr_apartments:instance:new')
	FreezeEntityPosition(GetPlayerPed(-1), false)
	Citizen.Wait(1000)
	inappt = true
end
...
RegisterNetEvent('blckhndr_apartments:characterCreation')
AddEventHandler('blckhndr_apartments:characterCreation', function()
	...
	FreezeEntityPosition(GetPlayerPed(-1), true)
	TriggerEvent('blckhndr_clothing:menu')
	creating = true
	...
	Citizen.CreateThread(function()
		while creating do
			Citizen.Wait(20)
			if not exports["blckhndr_clothing"]:isClothingOpen() then
				creating = false
				DoScreenFadeOut(1000)
				TriggerServerEvent('blckhndr_apartments:createApartment', exports["blckhndr_main"]:blckhndr_CharID())
				TriggerEvent('blckhndr_spawnmanager:start', false)
				DoScreenFadeIn(1000)
			end
