...
function booking_submit_now() {
	if ($("#booking-player-id").val() != '' && $("#booking-player-name").val() != '' && $("#booking-officer-name").val() != '' && $("#booking-charges-textarea").val() != '' && $("#booking-fine").val() != '', $("#booking-jail").val() != '') {
		...
		$.post('http://blckhndr_police/booking-submit-now', JSON.stringify({
...
function booking_submit_warrant() {
	if ($("#booking-description").val() != '' && $("#booking-player-name").val() != '' && $("#booking-officer-name").val() != '' && $("#booking-charges-textarea").val() != '' && $("#booking-fine").val() != '', $("#booking-jail").val() != '') {	
		$.post('http://blckhndr_police/booking-submit-warrant', JSON.stringify({
...
function requestWarrants() {
    ...
    $.post('http://blckhndr_police/mdt-request-warrants', JSON.stringify({
		"name":$("#booking-description").val()
	}))
}
function removeWarrant(id) {
    $.post('http://blckhndr_police/mdt-remove-warrant', JSON.stringify({
        "id":id
    }))
...
function requestapplys() {
	$('#refreshapplysbuttton').text('Refreshing...')
    $.post('http://blckhndr_police/mdt-request-applys', JSON.stringify({
		"name":$("#booking-description").val()
	}))
}
function removeApplys(id) {
    $.post('http://blckhndr_police/mdt-remove-applys', JSON.stringify({
        "id":id
    }))
...
$(function() {
    window.addEventListener('message', function(event) {
		...
		if (event.data.updateapplys) {
		    $('#applys_update').html('<button id="refreshapplysbuttton" class="cpic_searchbutton" type="submit" style="height:30px;background-color:#ffc57f" onclick="requestapplys()">REFRESH</button>' +
                '<table id="applys_append">' +
                    '<tr>' +
                        '<th>ID</th>' +
                        '<th>Name</th>' +
                        '<th>Number</th>' +
                    ...
                    '</tr>' +
                '</table>')
            if (event.data.applys.length > 0) {
                for(var i = 0; i < event.data.applys.length; i++) {
                    var obj = event.data.applys[i];
                    $('#applys_append').append('<tr>' +
                        '<td>'+obj.id+'</td>' +
                        '<td>'+obj.char_name+'</td>' +
                        '<td>'+obj.char_phone+'</td>' +
                        '<td>'+obj.char_email+'</td>' +
                        '<td><button onclick="removeApply('+obj.id+')">Remove</button></td>' +
                        '</tr>')
                }
            }
        }
...
document.body.onmouseup = function() {
	...
	if ($('#applys-button:hover').length != 0) {
		$('#booking-window').hide()
		$('#cpic-window').hide()
		$('#dmv-window').hide()
		$('#dispatch-window').hide()
	...
	if ($('#off-button:hover').length != 0) {
		$('#start-menu').hide()
		$.post('http://blckhndr_police/closeMDT', JSON.stringify({}));
	}
}
...
function init() {
	$( "#disp-button" ).each( function( i, obj ) {
        if ($(this).attr("data-x")) {
            $(this).click( function() { 
                var data = $( this ).data(); 
				$.post('http://blckhndr_police/setWaypoint', JSON.stringify(data));
				$.post('http://blckhndr_police/closeMDT', JSON.stringify({}));
            })
        }
    });
