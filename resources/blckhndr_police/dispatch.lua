...
RegisterNetEvent('blckhndr_police:dispatch:toggle')
AddEventHandler('blckhndr_police:dispatch:toggle', function()
  if disp_enable then
    disp_enable = false
    TriggerEvent('blckhndr_notify:displayNotification', 'Dispatch: <b style="color:red">DISABLED', 'centerRight', 4000, 'info')
  else
    disp_enable = true
    TriggerEvent('blckhndr_notify:displayNotification', 'Dispatch: <b style="color:#42f474">ENABLED', 'centerRight', 4000, 'info')
  end
end)
  ...
  if pdonduty then
    ...
    if disp_enable then
		if actions[id].name then
		  ...
		  if id == 7 then
			TriggerEvent('chatMessage', '', {255,255,255}, '^6^*:blckhndr_police:^0^r A BANK IS BEING ROBBED @ '..sname)
		  end
	...
	if actions[id].blip ~= false then
		...
		TriggerEvent("blckhndr_main:blip:add", "police", "ALERT: "..actions[id].tencode, actions[id].blip, x, y, z)
	end
  end
end

RegisterNetEvent('blckhndr_police:dispatchcall')
AddEventHandler('blckhndr_police:dispatchcall', function(tbl, id, chatPrint)
  if chatPrint then
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsPedShooting(GetPlayerPed(-1)) and exports["blckhndr_criminalmisc"]:HoldingWeapon() then
			print 'adding gsr'
			lastGSR = current_time
			myGSR = true
			TriggerEvent('blckhndr_evidence:ped:addState', 'GSR Residue', 'LFINGER')
		end
		if IsPedInMeleeCombat(GetPlayerPed(-1)) and not pdonduty then
			if last_Fight+10000 < GetGameTimer() then
			   ...
			   TriggerServerEvent('blckhndr_police:dispatch', coords, 15)
...
RegisterNetEvent('blckhndr_commands:police:gsrMe')
AddEventHandler('blckhndr_commands:police:gsrMe', function(pd)
	TriggerServerEvent('blckhndr_commands:police:gsrResult', pd, myGSR)
end)
...
Citizen.CreateThread(function()
   while true do
	 ...
	 if IsPedShooting(GetPlayerPed(-1)) and not pdonduty then
		if exports["blckhndr_criminalmisc"]:HoldingWeapon() then
		   local pos = GetEntityCoords(GetPlayerPed(-1))
		   local coords = {
			 x = pos.x,
			 y = pos.y,
		   ...
		   TriggerServerEvent('blckhndr_police:dispatch', coords, 1)
