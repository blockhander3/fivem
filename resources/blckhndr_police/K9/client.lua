...
RegisterNetEvent('blckhndr_police:ToggleK9')
AddEventHandler('blckhndr_police:ToggleK9', function(Model)
    if Toggle then
...
RegisterNetEvent('blckhndr_police:Sit')
AddEventHandler('blckhndr_police:Sit', function()
    if Sitting then
...
RegisterNetEvent('blckhndr_police:GetInVehicle')
AddEventHandler('blckhndr_police:GetInVehicle', function()
...
RegisterNetEvent('blckhndr_police:k9:search:person:me')
AddEventHandler('blckhndr_police:k9:search:person:me', function(OfficerId)
    local Drugs = blckhndr_getIllegalItems()
    local Found = false
    for Key, Value in pairs(Drugs) do
        if exports['blckhndr_inventory']:blckhndr_HasItem(Value) then
            Found = true
        end
    end
    TriggerServerEvent('blckhndr_police:k9:search:person:end', Found, OfficerId)
end)

RegisterNetEvent('blckhndr_police:k9:search:person:finish')
AddEventHandler('blckhndr_police:k9:search:person:finish', function(Found)
    if Found then
        ...
        TriggerEvent('blckhndr_notify:displayNotification', 'Your dog has detected an illegal substance', 'centerLeft', 2000, 'error')
        Citizen.Wait(2000)
        ClearPedTasksImmediately(K9)
        TaskFollowToOffsetOfEntity(K9, PlayerPedId(), 0.5, 0.0, 0.0, 5.0, -1, 1.0, 1)
        SetPedKeepTask(K9, true)
    else
        TriggerEvent('blckhndr_notify:displayNotification', 'Your dog did not detect anything', 'centerLeft', 2000, 'success')
    end
end)
