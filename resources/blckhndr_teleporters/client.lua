    ...
    for k, v in pairs(spots) do
      if GetDistanceBetweenCoords(v.enter.x, v.enter.y, v.enter.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
        ...
        if GetDistanceBetweenCoords(v.enter.x, v.enter.y, v.enter.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
          if v.locked then
            blckhndr_drawText3D(v.enter.x, v.enter.y, v.enter.z, "~r~"..k.." is locked!")
          else
            if IsControlJustPressed(0, 51) then
			  local canenter = true
			  if not exports["blckhndr_police"]:blckhndr_PDDuty() then
				  if k == 'Courtroom' then
					for k, v in pairs(savingWeapons) do
						if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(v)) and canenter then
							TriggerEvent('blckhndr_notify:displayNotification', 'No weapons in the courtroom.', 'centerLeft', 4000, 'error')
							canenter = false
						end
					end
				  end
      ...
      if GetDistanceBetweenCoords(v.exit.x, v.exit.y, v.exit.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
        ...
        if GetDistanceBetweenCoords(v.exit.x, v.exit.y, v.exit.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
          blckhndr_drawText3D(v.exit.x, v.exit.y, v.exit.z, "Press [E] to exit "..k)
          if IsControlJustPressed(0, 51) then
            DoScreenFadeOut(1000)
            Citizen.Wait(1500)
...
RegisterNetEvent('blckhndr_doj:judge:toggleLock')
AddEventHandler('blckhndr_doj:judge:toggleLock', function(tf)
	spots["Courtroom"].locked = tf
end)

--SetEntityCoords(GetPlayerPed(-1), -458.790, 284.750, 78.521)

RegisterNetEvent('blckhndr_teleporters:teleport:waypoint')
AddEventHandler('blckhndr_teleporters:teleport:waypoint', function(source)
...
RegisterNetEvent('blckhndr_teleporters:teleport:coordinates')
AddEventHandler('blckhndr_teleporters:teleport:coordinates', function(x, y, z)
