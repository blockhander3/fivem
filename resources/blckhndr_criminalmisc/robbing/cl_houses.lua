			...
			if GetDistanceBetweenCoords(craft_spot.x,craft_spot.y,craft_spot.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
				...
				if crafting then
					...
					if IsControlJustPressed(0,38) then
						if exports["blckhndr_inventory"]:blckhndr_HasItem('aluminium') and exports["blckhndr_inventory"]:blckhndr_HasItem('steel') then
							if exports["blckhndr_inventory"]:blckhndr_GetItemAmount('aluminium') < 2 then
								exports['mythic_notify']:DoCustomHudText('error', 'You need 2 aluminium', 4000)					
								return
							end
							if exports["blckhndr_inventory"]:blckhndr_GetItemAmount('steel') < 5 then
							...
							exports["blckhndr_progress"]:blckhndr_ProgressBar(58, 133, 255,'Crafting',5)
							TriggerEvent('blckhndr_inventory:item:take', 'aluminium', 2)
							TriggerEvent('blckhndr_inventory:item:take', 'steel', 5)
						else
...
RegisterNetEvent('blckhndr_criminalmisc:houserobbery:try')
AddEventHandler('blckhndr_criminalmisc:houserobbery:try', function()
	local tyme = exports["blckhndr_timeandweather"]:getTime()
	...
	if exports["blckhndr_inventory"]:blckhndr_HasItem('crowbar') then
		...
		if robbing_id then
			...
			TriggerServerEvent('blckhndr_police:dispatch', coords, 18, 'I think /'..robbables[robbing_id].info..'/ is currently being broken into!')
								...
								if GetDistanceBetweenCoords(v.xyz.x, v.xyz.y, v.xyz.z, GetEntityCoords(GetPlayerPed(-1)),true) < 1 then
									if not searching then
										...
										if IsControlJustPressed(0,38) then
											...
											exports["blckhndr_progress"]:blckhndr_ProgressBar(58, 133, 255,'Looting',5)
											TriggerEvent('blckhndr_needs:stress:add', 3)
											Citizen.CreateThread(function()
												while searching do Citizen.Wait(0)
													...
													if GetDistanceBetweenCoords(spot.xyz.x,spot.xyz.y,spot.xyz.z,GetEntityCoords(GetPlayerPed(-1)),true) < 1 then 
														if searching_start+5000 <= GetGameTimer() then
															...
															if v.cash > 0 then
																TriggerEvent('blckhndr_bank:change:walletAdd', math.random(v.cash / 2, v.cash * 2))
															end
															for _,itm in pairs(v.items) do
																if math.random(1,100) < 40 then
																	if itm.preset then
																		TriggerEvent('blckhndr_inventory:items:addPreset', itm.item, math.random(1,3))
																	end
																end
															end
															for i=1,math.random(1,3) do
																...
																item.amt = 1
																TriggerEvent('blckhndr_inventory:items:add', item)
															end
														end
													else
														exports["blckhndr_progress"]:removeBar()
