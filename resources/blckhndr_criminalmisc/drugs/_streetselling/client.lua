...
local init = false
local areas = {}
RegisterNetEvent('blckhndr_criminalmisc:drugs:streetselling:send')
AddEventHandler('blckhndr_criminalmisc:drugs:streetselling:send', function(tbl)
	init = true
	areas = tbl
	for k, v in pairs(tbl) do
		if v.premium then
			if exports["blckhndr_inventory"]:blckhndr_HasItem('radio_receiver') then
				--[[
				TriggerEvent('blckhndr_phone:recieveMessage', {
				  sender = 'DarkWeb',
				  from_number = 666,
				  to_number = -1,
				...
				TriggerEvent('blckhndr_phones:USE:Email', {
...
Citizen.CreateThread(function()
	TriggerServerEvent('blckhndr_criminalmisc:drugs:streetselling:request')
end)

----------------------------------------------- system xx
function hasDrugs()
	for k, v in pairs(sellables) do
		if exports["blckhndr_inventory"]:blckhndr_HasItem(v) then
			return true
		end
	end
	return false
...
function getPed()
	local peds = exports.blckhndr_entfinder:getPeds(false)
	if peds then
	...
	while true do
		...
		if GetDistanceBetweenCoords(radioxyz.x, radioxyz.y, radioxyz.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
			...
			if GetDistanceBetweenCoords(radioxyz.x, radioxyz.y, radioxyz.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
				...
				if IsControlJustPressed(0, 38) then
					TriggerServerEvent('blckhndr_criminalmisc:drugs:streetselling:radio')
				end
			end
		end
		if init and hasDrugs() and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if areas[myarea] and areas[myarea].enabled and areas[myarea].drugs[drug] then
				local obj = getPed()--blckhndr_FindNearbyPed(1.5)
				if obj and IsPedHuman(obj) and not table.contains(sold_peds, obj) and not IsEntityDead(obj) and not IsPedAPlayer(obj) then
					if not selling then
						if IsControlPressed(0, 19) then
							blckhndr_drawText3D(GetEntityCoords(obj).x, GetEntityCoords(obj).y, GetEntityCoords(obj).z, '[~g~E~w~] Sell ~b~'..drug)
							if IsControlJustPressed(0, 38) then
								if math.random(1,100) > areas[myarea].callpercentage then
									...
									TriggerServerEvent('blckhndr_police:dispatch', coords, 3)
							...
							if math.random(0,100) > areas[myarea].sellpercentage then
							  ...
							  TriggerEvent('blckhndr_notify:displayNotification', 'They are not interested...', 'centerLeft', 3000, 'error')
							  TriggerEvent('blckhndr_needs:stress:add', 10)
						...
						if selltime > curtime then
							blckhndr_drawText3D(GetEntityCoords(obj).x, GetEntityCoords(obj).y, GetEntityCoords(obj).z, 'Selling')
							TaskStandStill(obj, 9000)
						else
							blckhndr_drawText3D(GetEntityCoords(obj).x, GetEntityCoords(obj).y, GetEntityCoords(obj).z, 'Sold')
							...
							if finishtime < curtime then
								...
								-- work out amount
								if exports["blckhndr_inventory"]:blckhndr_GetItemAmount(selling_item) < areas[myarea].drugs[drug].avg then
								  sold_amount = exports["blckhndr_inventory"]:blckhndr_GetItemAmount(selling_item)
								else
								...
								-- state stuff
								if drug == 'joint' then
									TriggerEvent('blckhndr_evidence:ped:addState', 'Slight aroma of weed', 'UPPER_BODY', 300)
								elseif drug == 'meth_rocks' then
									TriggerEvent('blckhndr_evidence:ped:addState', 'Empty packet with\nmeth residue\nhanging out of\npocket...', 'UPPER_BODY', 60)
								end
								...
								TriggerEvent('blckhndr_notify:displayNotification', 'They bought '..sold_amount..' '..exports["blckhndr_inventory"]:blckhndr_GetItemDetails(drug).name..' for '..price..'DM', 'centerLeft', 3000, 'info')
								TriggerEvent('blckhndr_inventory:item:add', 'dirty_money', price)
								TriggerEvent('blckhndr_inventory:item:take', selling_item, sold_amount)
								TriggerEvent('blckhndr_needs:stress:add', 10)
					...
					if selling then
						TriggerEvent('blckhndr_notify:displayNotification', 'The transaction was cancelled...', 'centerLeft', 3000, 'error')
						selling = not selling
					end
				end
			else
