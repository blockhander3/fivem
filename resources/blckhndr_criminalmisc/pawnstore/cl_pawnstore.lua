				...
				if GetDistanceBetweenCoords(loc.x, loc.y, loc.z, GetEntityCoords(GetPlayerPed(-1)), false) < 1 then
					...
					if IsControlJustPressed(0,38) then
						for k,v in pairs(unique_items) do
							if exports["blckhndr_inventory"]:blckhndr_HasItem(v.index) then
								local amt = exports["blckhndr_inventory"]:blckhndr_GetItemAmount(v.index)
								local price = math.random(100,300)
								TriggerEvent('blckhndr_inventory:item:take', v.index, amt)
								TriggerEvent('blckhndr_bank:change:walletAdd', amt*price)
							end
						end
					end
				end
