      ...
      if GetDistanceBetweenCoords(v.loc.x,v.loc.y,v.loc.z,GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
        ...
        if GetDistanceBetweenCoords(v.loc.x,v.loc.y,v.loc.z,GetEntityCoords(GetPlayerPed(-1)), true) < 4 then
          ...
          if IsControlJustPressed(0,38) then
            if exports.blckhndr_main:blckhndr_GetWallet() >= v.cost then
              if exports.blckhndr_inventory:blckhndr_GetItemAmount(v.required.item) >= v.required.amount then
                local waittime = math.random(4000,10000)
                TriggerEvent('blckhndr_notify:displayNotification', 'Gathering ['..v.gives.amount..'X] '..v.gives.itemtitle, 'centerLeft', waittime, 'info')
                Citizen.Wait(waittime)
				if v.state then
					print('addingstate: '..v.state)
					TriggerEvent('blckhndr_evidence:ped:addState', v.state, 'UPPER_BODY', 120)
				end
                if v.required.item ~= 'none' then
                  TriggerEvent('blckhndr_inventory:item:take', v.required.item, v.required.amount)
                end
                TriggerEvent('blckhndr_notify:displayNotification', 'Gathered ['..v.gives.amount..'X] '..v.gives.itemtitle, 'centerRight', 3500, 'success')
                if v.cost > 0 then
                  TriggerEvent('blckhndr_bank:change:walletMinus', v.cost)
                end
                TriggerEvent('blckhndr_inventory:item:add', v.gives.item, v.gives.amount)
              else
                TriggerEvent('blckhndr_notify:displayNotification', 'You don\'t have any '..v.required.itemtitle, 'centerRight', 3500, 'error')
              end
            else
              TriggerEvent('blckhndr_notify:displayNotification', 'You don\'t have $'..v.cost..'!!!', 'centerRight', 3500, 'error')
            end
          end
        end
      end
