...
function gunrange_start()
  for k, v in pairs(ranges) do
    if v.inuse then
      print(':blckhndr_shootingrange: Range #'..k..' is in use')
    else
      print(':blckhndr_shootingrange: Range #'..k..' is not in use, joining range')
      gunrange_join(k)
      return
    end
  end
  TriggerEvent('blckhndr_notify:displayNotification', 'There are no ranges available, try again later.', 'centerLeft', 4000, 'error')
end
...
function gunrange_CreateTarget(lastid)
  ...
  if targets[lastid] or lastid == 0 then
    ...
    TriggerEvent('blckhndr_notify:displayNotification', 'Target created...', 'centerRight', 3000, 'info')
  else
    print('no target')
  end
end
