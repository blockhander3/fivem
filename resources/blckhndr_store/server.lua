...
RegisterServerEvent('blckhndr_store:request')
AddEventHandler('blckhndr_store:request', function(store_id, gunstore)
            ...
            for k, v in pairs(s.stock) do
                if items[k] then
                    ...
                    table.insert(inv, #inv+1, item)
                     ...
                     print('ERROR (blckhndr_store) :: Item '..k..' is not defined in server.lua')
                end
            end

            TriggerClientEvent('blckhndr_inventory:store:recieve', source, store_id, inv, gunstore)
         else
            TriggerClientEvent('blckhndr_notify:displayNotification', source, 'This store is in use by another player: '..s.busy, 'centerRight', 8000, 'error')
        end
	else
		TriggerClientEvent('blckhndr_notify:displayNotification', source, 'ERROR: This store does not seem to exist', 'centerRight', 8000, 'error')
	end
end)

RegisterServerEvent('blckhndr_store:boughtOne')
AddEventHandler('blckhndr_store:boughtOne', function(store_id, item)
	local s = stores[store_id]
	if s then
...
RegisterServerEvent('blckhndr_store:closedStore')
AddEventHandler('blckhndr_store:closedStore', function(store_id)
	local s = stores[store_id]
	if s then
		s.busy = false
	else 
...
RegisterNetEvent('blckhndr_store:recieveItemsForStore')
AddEventHandler('blckhndr_store:recieveItemsForStore', function(presetItems)
...
AddEventHandler('onResourceStart', function(resourceName)
  ...
  TriggerEvent('blckhndr_inventory:sendItemsToStore')
end)