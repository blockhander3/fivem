local stores = {
	...
	{id = 'robsliquor', xyz = vector3(1392.7415771484, 3604.9301757812, 34.980926513672), blip=true, busy = false, gunstore = false},
					...
					if not s.busy then
						...
						if IsControlJustReleased(0, Util.GetKeyNumber('E')) then
							if not s.gunstore then
								TriggerServerEvent('blckhndr_store:request', s.id, s.gunstore)
								print('No license required')
							elseif s.gunstore then
								if requireLicense and not exports["blckhndr_licenses"]:blckhndr_hasLicense('weapon') then
									TriggerEvent('blckhndr_notify:displayNotification', 'This store requires a <b>WEAPON</b> license!', 'centerLeft', 3000, 'error')
								else
					...
					if IsPlayerFreeAiming(PlayerId()) and IsPlayerFreeAimingAtEntity(PlayerId(), v.ped) then
						if robbing then
							...
							if maff < curtime then
								robbing = false
								TriggerEvent('blckhndr_bank:change:walletAdd', math.random(100, 600))
								TriggerEvent('blckhndr_inventory:item:add', 'dirty_money', math.random(500,1000))
								Citizen.Wait(4000)
							end
						elseif not IsPedInCombat(v.ped) then
							...
							if quickmaff < curtime or lastrob == 0 then
								if math.random(0,100) > 20 then
									...
									robbing = true
									TriggerEvent('blckhndr_notify:displayNotification', 'Robbing...', 'centerLeft', 6000, 'info')
									exports["blckhndr_progress"]:blckhndr_ProgressBar(58, 133, 255,'ROBBING',30)
									...
									if math.random(0,100) > 30 then
										...
										TriggerServerEvent('blckhndr_police:dispatch', coords, 12, '10-31b | ARMED ROBBERY IN PROGRESS')
									end
								else
								...
								TriggerEvent('blckhndr_notify:displayNotification', 'You can\'t do that yet!', 'centerLeft', 6000, 'info')
								local pos = GetEntityCoords(PlayerPedId())
								local coords = {
								 x = pos.x,
								 y = pos.y,
								...
								TriggerServerEvent('blckhndr_police:dispatch', coords, 12, '10-31b | Attempted armed store robbery')
								Citizen.Wait(6000)
							end
						end
					else
						if robbing then
							...
							TriggerEvent('blckhndr_notify:displayNotification', 'You failed...', 'centerLeft', 6000, 'error')
							robbing = false
							Citizen.Wait(6000)
						end
					end
...
Citizen.CreateThread(function()
	...
	while true do Citizen.Wait(0)
		for k,v in pairs(storekeepers) do
			if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(PlayerPedId()), true) < 10 then
				if exports["blckhndr_entfinder"]:getPedNearCoords(v.x, v.y, v.z,5) then
					if GetEntityModel(exports["blckhndr_entfinder"]:getPedNearCoords(v.x, v.y, v.z,5)) == 416176080 then
						v.ped = exports["blckhndr_entfinder"]:getPedNearCoords(v.x, v.y, v.z,5)
					...
					--v.ped = exports['blckhndr_main']:blckhndr_FindPedNearbyCoords(v.x, v.y, v.z,5)
				else
					if not v.ped then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist < 2.0 then
			...
			money = exports.blckhndr_main:blckhndr_GetWallet()
		
			if IsControlJustPressed(0, 38) then
				if money >= weaponLicenseprice then
					if not exports["blckhndr_licenses"]:blckhndr_hasLicense('weapon') then
						TriggerEvent('blckhndr_licenses:police:give', 'weapon')
						--TriggerEvent('blckhndr_notify:displayNotification', 1, 'You were granted a <b>Weapons License</b> with 0 infractions', 'centerRight', 7000, 'info')
						exports['mythic_notify']:DoCustomHudText('success', 'You bought a weapons license with 0 infractions!', 4000)
						TriggerEvent('blckhndr_bank:change:walletMinus', weaponLicenseprice)
					else
						--TriggerEvent('blckhndr_notify:displayNotification', 1, 'You already have a weapons license', 'centerRight', 7000, 'info')
