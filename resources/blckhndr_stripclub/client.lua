...
RegisterNetEvent('blckhndr_stripclub:client:update')
AddEventHandler('blckhndr_stripclub:client:update', function(tbl)
  positions = tbl
end)
...
function StartLapDance(id)
  if positions[id].inuse then
    TriggerEvent('blckhndr_notify:displayNotification', 'This booth is already in use numnuts', 'centerLeft', 3000, 'error')
  else
    curbooth = id
    TriggerServerEvent('blckhndr_stripclub:server:claimBooth', id)
    ...
    TriggerEvent('blckhndr_emotecontrol:play', 'anim', 'switch@michael@sitting', 'idle')
