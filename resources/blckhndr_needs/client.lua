...
function blckhndr_thirst()
	return thirst
end

function blckhndr_hunger()
	return hunger
end

function blckhndr_stress()
	return stress
end
...
AddEventHandler('blckhndr_inventory:initChar', function()
  init = true
	hunger = 0
	thirst = 0
	stress = 0
...
local paused = false
RegisterNetEvent('blckhndr_hungerandthirst:pause')
RegisterNetEvent('blckhndr_hungerandthirst:unpause')
AddEventHandler('blckhndr_hungerandthirst:pause', function()
	paused = true
	TriggerEvent('blckhndr_notify:displayNotification', 'Hunger, thirst, and stress usage has been paused', 'centerRight', 3000, 'info')
end)
AddEventHandler('blckhndr_hungerandthirst:unpause', function()
	TriggerEvent('blckhndr_notify:displayNotification', 'Hunger, thirst, and stress usage has been reactivated', 'centerRight', 3000, 'info')
	paused = false
end)
    ...
    if clienttime / 1000 == math.floor(clienttime) / 1000 and init then
      if hunger - 0.006 <= 0 then
		...
		if helf < 105 and not ded then
			...
			TriggerEvent('blckhndr_notify:displayNotification', 'Elfelejtettél enni!', 'centerLeft', 5000, 'error')
			ded = true
		else
			if not ded then
			...
			if notifstarve == false then
				TriggerEvent('blckhndr_notify:displayNotification', '<b>ÉHEZEL', 'centerLeft', 3000, 'info')
				notifstarve = true
			end
		end
      else
      ...
      if thirst - 0.007 <= 0 then
		...
		if helf < 105 and not ded then
			TriggerEvent('blckhndr_ems:killMe')
			TriggerEvent('blckhndr_notify:displayNotification', 'Elfelejtettél inni!', 'centerLeft', 5000, 'error')
			ded = true
		else
			if not ded then
				SetEntityHealth(GetPlayerPed(-1), GetEntityHealth(GetPlayerPed(-1))-2)
			...
			if notifthirst == false then
				TriggerEvent('blckhndr_notify:displayNotification', '<b>SZOMJAZOL', 'centerLeft', 3000, 'info')
				notifthirst = true
			end
		end
      else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedShooting(playerPed) and not exports.blckhndr_police:blckhndr_PDDuty() and not blacklisted() then -- If its just a civ and they are not using a blacklisted weapon
			TriggerEvent('blckhndr_needs:stress:add', 1)
		elseif IsPedShooting(playerPed) and exports.blckhndr_police:blckhndr_PDDuty() and not blacklisted() then -- If police are on duty they get less stress and not using a blacklisted weapon
			TriggerEvent('blckhndr_needs:stress:add', 0.1)
		end
	end
end)
...
RegisterNetEvent('blckhndr_inventory:use:food')
AddEventHandler('blckhndr_inventory:use:food', function(relief)
...
RegisterNetEvent('blckhndr_inventory:use:drink')
AddEventHandler('blckhndr_inventory:use:drink', function(relief)
...
RegisterNetEvent('blckhndr_needs:stress:add')
AddEventHandler('blckhndr_needs:stress:add', function(increase)
...
RegisterNetEvent('blckhndr_needs:stress:remove')
AddEventHandler('blckhndr_needs:stress:remove', function(relief)
