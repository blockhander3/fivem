...
RegisterServerEvent('blckhndr_gangs:request')
AddEventHandler('blckhndr_gangs:request', function()
	TriggerClientEvent('blckhndr_gangs:recieve', source, config)
end)

RegisterServerEvent('blckhndr_gangs:inventory:request')
AddEventHandler('blckhndr_gangs:inventory:request', function(key)
	if config[key] then
		local g = config[key]
...
RegisterServerEvent('blckhndr_gangs:tryTakeOver')
AddEventHandler('blckhndr_gangs:tryTakeOver', function(key)
	if config[key] then
		local g = config[key]
		local online_members = {}
		for _,m in pairs(g.members) do
			if export['blckhndr_main']:blckhndr_GetPlayerFromCharacterId(m.id) and NetworkIsPlayerActive(blckhndr_GetPlayerFromCharacterId(m.id)) then
				table.insert(online_members, #online_members+1, export['blckhndr_main']:blckhndr_GetPlayerFromCharacterId(m.id))
			end
		end
		if #online_members < script.required_online then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Nincs elég bandatag'})
			return
		end
		if not g.takeover.ongoing then

		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Valaki már probálja elfoglalni'})
			return
		end
	end
end)

RegisterServerEvent('blckhndr_gangs:garage:enter')
AddEventHandler('blckhndr_gangs:garage:enter', function(key, mdl, plate)
	if config[key] then
		local g = config[key]
		if not g.interior.car then	
			...
			TriggerClientEvent('blckhndr_gangs:hideout:enter', source, key, config[key])
		else	
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Már van egy jármű a garázsban.'})
		end
	end
end)

RegisterServerEvent('blckhndr_gangs:hideout:enter')
AddEventHandler('blckhndr_gangs:hideout:enter', function(key)
	if config[key] then
		local g = config[key]
...
RegisterServerEvent('blckhndr_gangs:hideout:leave')
AddEventHandler('blckhndr_gangs:hideout:leave', function(key, incar)
	if config[key] then
		local g = config[key]
		if incar then
			-- do car shenanigans
			TriggerClientEvent('blckhndr_gangs:hideout:leave', source, g.interior.car.plate)
			g.interior.car = false
		else
			TriggerClientEvent('blckhndr_gangs:hideout:leave', source)
		end
	end
end)