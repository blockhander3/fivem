...
function doPriority()
	...
	MySQL.Async.fetchAll('SELECT * FROM `blckhndr_users` WHERE `priority` != 0', {}, function(res)
		for k, usr in pairs(res) do
			Queue.AddPriority(usr.steamid, tonumber(usr.priority))
			print(':blckhndr_priority: Adding priority('..usr.priority..') for user '..usr.name..'('..usr.steamid..')')
		end
	end)
	prio = true
end
