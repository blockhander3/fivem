...
RegisterNetEvent('blckhndr_vehiclecontrol:damage:repair')
AddEventHandler('blckhndr_vehiclecontrol:damage:repair', function()
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				TriggerEvent('blckhndr_notify:displayNotification', 'You repaired this vehicle.', 'centerLeft', 3000, 'success')
			else
				TriggerEvent('blckhndr_notify:displayNotification', 'You cannot repair this vehicle, contact a tow truck.', 'centerLeft', 3000, 'error')
			end
		else
			TriggerEvent('blckhndr_notify:displayNotification', 'This vehicle does not need repairing.<br>You <b>wasted</b> a repair kit!', 'centerLeft', 4000, 'error')
		end
	else
...
RegisterNetEvent('blckhndr_vehiclecontrol:damage:repairkit')
AddEventHandler('blckhndr_vehiclecontrol:damage:repairkit', function()
	if IsPedInAnyVehicle(GetPlayerPed(-1)) then
		TriggerEvent('blckhndr_notify:displayNotification', 'You need to be outside of the vehicle to use this.', 'centerLeft', 4000, 'error')
	else
		if blckhndr_lookingAt() then
			local vehicle = blckhndr_lookingAt()
			...
			if GetDistanceBetweenCoords(moveto, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
				...
				exports["blckhndr_progress"]:blckhndr_ProgressBar(58, 133, 255,'repairing',10)
				...
				if GetDistanceBetweenCoords(moveto, GetEntityCoords(GetPlayerPed(-1)), true) < 2 and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
					...
					SetVehicleEngineOn(vehicle, true, false )
					SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
					TriggerEvent('blckhndr_notify:displayNotification', 'You repaired this vehicle.', 'centerLeft', 4000, 'success')
					--TriggerEvent('blckhndr_inventory:item:take', 'repair_kit', 1)
				else
					TriggerEvent('blckhndr_notify:displayNotification', 'You moved away from the vehicle!', 'centerLeft', 4000, 'error')
				end
			else
				TriggerEvent('blckhndr_notify:displayNotification', 'Move to the front of the vehicle', 'centerLeft', 4000, 'error')
			end
		else	
			TriggerEvent('blckhndr_notify:displayNotification', 'No vehicle detected', 'centerLeft', 4000, 'error')
		end
	end
	--[[
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				TriggerEvent('blckhndr_notify:displayNotification', 'You cannot repair this vehicle, contact a tow truck.', 'centerLeft', 4000, 'error')
			end
		else
			TriggerEvent('blckhndr_notify:displayNotification', 'This vehicle does not need repairing.', 'centerLeft', 4000, 'error')
		end
	else
		notification("~y~You must be in a vehicle to be able to repair it")
	end
