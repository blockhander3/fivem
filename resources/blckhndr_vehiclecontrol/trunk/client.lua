...
RegisterNetEvent('blckhndr_vehiclecontrol:trunk:forceIn')
AddEventHandler('blckhndr_vehiclecontrol:trunk:forceIn', function(netid)
...
RegisterNetEvent('blckhndr_vehiclecontrol:trunk:forceOut')
AddEventHandler('blckhndr_vehiclecontrol:trunk:forceOut', function()
	if intrunk then
			...
			if veh and not intrunk then
				if not DoesVehicleHaveDoor(veh, 6) and DoesVehicleHaveDoor(veh, 5) and IsThisModelACar(GetEntityModel(veh)) and GetVehicleDoorsLockedForPlayer(veh, PlayerId()) == false then
					...
					if GetDistanceBetweenCoords(troonk.x, troonk.y, troonk.z, GetEntityCoords(GetPlayerPed(-1)), true) < 1 then
						if exports["blckhndr_ems"]:carryingWho() ~= 0  then
							Util.DrawText3D(troonk.x, troonk.y, troonk.z, '[E] Put '..exports["blckhndr_ems"]:carryingWho()..' in trunk', {255, 255, 255, 140}, 0.3)
							if IsControlJustPressed(0,38) then
								TriggerServerEvent('blckhndr_ems:carry:end', exports["blckhndr_ems"]:carryingWho())
								TriggerServerEvent('blckhndr_vehiclecontrol:trunk:forceIn', exports["blckhndr_ems"]:carryingWho(), NetworkGetNetworkIdFromEntity(veh))
							end
						else
							...
							if IsControlJustPressed(0,74) then
								TriggerServerEvent('blckhndr_inventory:veh:request', GetVehicleNumberPlateText(veh), 'trunk')
							end
						end
					end
				end
