...
canrob = true
RegisterNetEvent('blckhndr_bankrobbery:timer')
AddEventHandler('blckhndr_bankrobbery:timer', function(state)
	if canrob ~= state then
		canrob = state
		print('blckhndr_bankrobbery: canrob = '..tostring(state))
	end
end)
...
local bankdoors = {
  ...
  -- PALETO
  [1] = {hash=-1185205679,x=-104.604,y=6473.443,z=31.79,tx=-104.604,ty=6473.443,tz=31.79,unlocked=false,unlocktime=0,keypad={
    x = -105.55908966064,
    y = 6471.998046875,
    z = 31.626714706421,
    crackattempts = 0,
  ...
  -- RT68
  [2] = {hash=2121050683,x=1175.542,y=2710.861,z=38.22,tx=1175.542,ty=2710.861,tz=38.22,unlocked=false,unlocktime=0,keypad={
    x = 1175.9526367188,
    y = 2712.880859375,
    z = 38.088054656982,
    crackattempts = 0,
...
AddEventHandler('blckhndr_main:character', function()
  TriggerServerEvent('blckhndr_bankrobbery:init')
end)

RegisterNetEvent('blckhndr_bankrobbery:init')
AddEventHandler('blckhndr_bankrobbery:init', function(tbl)
  for k, v in pairs(tbl) do
    local door = bankdoors[k]
    if v[1] == 'open' then
      door.status.status = v[1]
      TriggerEvent('blckhndr_bankrobbery:openDoor', k)
    elseif v[1] == 'closed' then
      door.status.status = v[1]
      TriggerEvent('blckhndr_bankrobbery:closeDoor', k)
    end
  end
end)

RegisterNetEvent('blckhndr_bankrobbery:openDoor')
AddEventHandler('blckhndr_bankrobbery:openDoor', function(id)
...
RegisterNetEvent('blckhndr_bankrobbery:closeDoor')
AddEventHandler('blckhndr_bankrobbery:closeDoor', function(id)
  local door = bankdoors[id]
  local _door = GetClosestObjectOfType(door.x, door.y, door.z, 1.0, door.hash, false, false, false)
  if GetDistanceBetweenCoords(GetEntityCoords(_door), GetEntityCoords(GetPlayerPed(-1))) < 10 then
    --PlaySound(-1, "DOOR_OPEN", "CABLE_CAR_SOUNDS", 0, 0, 1)
  ...
  while true do
    ...
    if cracking then
      ...
      if rem < current_time then
        ...
        TriggerEvent('blckhndr_notify:displayNotification', 'Keypad cracking succeeded code: <b>'..door.keypad.code, 'centerLeft', 5000, 'success')
            ...
            if UpdateOnscreenKeyboard() == 1 then
              ...
              if codeinput == door.keypad.code then
                ...
                if door.keypad.crackattempts > 3 then
                  ...
                  TriggerServerEvent('blckhndr_police:dispatch', coords, 7)
                end
                DisplayOnscreenKeyboard(false, "FMMC_KEY_TIP8", "", "", "", "", "", door.keypad.difficulty)
                editOpen = true
    ...
    for k, door in pairs(bankdoors) do
      if GetDistanceBetweenCoords(door.x, door.y, door.z, GetEntityCoords(GetPlayerPed(-1)), true) < 40 then
          ...
          if GetDistanceBetweenCoords(door.vault.x, door.vault.y, door.vault.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
            if door.vault.robbed == false then
              ...
              if IsControlJustPressed(0,38) then
				if exports['blckhndr_inventory']:blckhndr_HasItem('drill') then
					if exports['blckhndr_inventory']:blckhndr_HasItem('modified_drillbit') then
						TriggerServerEvent('blckhndr_bankrobbery:payout', k, exports['blckhndr_inventory']:blckhndr_HasItem('modified_drillbit'))
					else
						if math.random(1,100) > 50 then
							TriggerEvent('blckhndr_inventory:item:take', 'drill', 1)
					...
					TriggerEvent('blckhndr_notify:displayNotification', 'You don\'t have anything to open the vault with...', 'centerLeft', 5000, 'error')
				end
              end
            else
              SetTextComponentFormat("STRING")
        ...
        if door.unlocked then
          ...
          door.keypad.code = 0
          TriggerEvent('blckhndr_notify:displayNotification', 'You no longer have access to the vault!', 'centerRight', 5000, 'error')
          TriggerServerEvent('blckhndr_bankrobbery:vault:close', k)
        end
      end
      if GetDistanceBetweenCoords(door.x, door.y, door.z, GetEntityCoords(GetPlayerPed(-1)), true) < 10 then
        local _door = GetClosestObjectOfType(door.x, door.y, door.z, 1.0, door.hash, false, false, false)
        local CurrentHeading = GetEntityHeading(_door)
        if door.unlocked == true or exports.blckhndr_police:blckhndr_PDDuty() then
          --[[
          blckhndr_drawText3D(door.tx,door.ty,door.tz+0.5, '~y~Hold [UP ARROW] to open door\nDEBUG: '..CurrentHeading)
          ...
          if GetDistanceBetweenCoords(door.x, door.y, door.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
            if door.status.status == 'open' then
              blckhndr_drawText3D(door.tx,door.ty,door.tz, 'Press [E] to ~r~close~w~ the vault!')
              if IsControlJustPressed(1,51) then
                TriggerServerEvent('blckhndr_bankrobbery:vault:close', k)
              end
            else
              blckhndr_drawText3D(door.tx,door.ty,door.tz, 'Press [E] to ~g~open~w~ the vault!')
              if IsControlJustPressed(1,51) then
                TriggerServerEvent('blckhndr_bankrobbery:vault:open', k)
              end
            end
          end
        else
          if GetDistanceBetweenCoords(door.keypad.x, door.keypad.y, door.keypad.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
            if exports['blckhndr_police']:blckhndr_getCopAmt() >= 3 then
              ...
              if IsControlJustPressed(1, 26) and not cracking then
          			...
          			TriggerServerEvent('blckhndr_police:dispatch', coords, 7)
					TriggerServerEvent('blckhndr_bankrobbery:start')
              ...
              SetTextComponentFormat("STRING")
              AddTextComponentString("~r~Not enough cops for a heist ("..exports['blckhndr_police']:blckhndr_getCopAmt().."/3)")
              DisplayHelpTextFromStringLabel(0, 0, 1, -1)
            end
          else
            if door.keypad.code ~= 0 then
              TriggerEvent('blckhndr_notify:displayNotification', 'You walked away therefor the bank has reset the code!', 'centerLeft', 5000, 'error')
              door.keypad.crackattempts = 0
              door.keypad.code = 0
            end
            if cracking then
              TriggerEvent('blckhndr_notify:displayNotification', 'Keypad cracking failed', 'centerLeft', 5000, 'error')
              cracking = false
              start_time = 0
            end
          end
...
local start = 0
RegisterNetEvent('blckhndr_bankrobbery:LostMC:spawn')
AddEventHandler('blckhndr_bankrobbery:LostMC:spawn', function()
	--[[ bye bye mc spawns
	local mypeds = {}
	...
	while true do
		Citizen.Wait(0)
		local ped = exports["blckhndr_entfinder"]:getPedNearCoords(LostMC.x, LostMC.y, LostMC.z, 100)
				...
				if DoesEntityExist(exploded_truck) then
					...
					if GetDistanceBetweenCoords(2701.4072265625, 3454.7998046875, 56.821979522705, GetEntityCoords(GetPlayerPed(-1))) < 0.5 and lockpicked then
						...
						if IsControlJustPressed(0, 38) then
							...
							TriggerEvent('blckhndr_inventory:item:add', 'drill', 1)
						end
					end
				else
...
-----------------------
-- Timer, I really need to confine all these to 1 in blckhndr_main - todo
-----------------------
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(1000)
