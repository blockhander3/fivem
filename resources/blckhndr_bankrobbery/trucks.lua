    ...
    if moneyTrucks[i]["breached"] then
      ...
      if HasPickupBeenCollected(cashPickup[i]) then
        if i == 0 or i == 1  or i > 1 then
		  ...
		  TriggerEvent('blckhndr_notify:displayNotification', 'You robbed the truck for '..payOut..'DM', 'centerLeft', 6000, 'info')
          --TriggerServerEvent('fs_freemode:missionComplete', payOut)
          TriggerEvent('blckhndr_inventory:item:add', 'dirty_money', payOut)
