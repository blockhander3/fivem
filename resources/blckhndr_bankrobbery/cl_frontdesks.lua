...
TriggerServerEvent('blckhndr_bankrobbery:desks:request')
				...
				if GetDistanceBetweenCoords(keyboard.robspot.x, keyboard.robspot.y, keyboard.robspot.z, GetEntityCoords(GetPlayerPed(-1)), true) < 0.5 then
					if keyboard.robbed == 'nothacked' then
						...
						if IsControlJustPressed(0,38) then
							TriggerServerEvent('blckhndr_bankrobbery:desks:startHack', k, key)
							
							local function afterHack(success, timeremaining)
								if success then
									TriggerServerEvent('blckhndr_bankrobbery:desks:endHack', k, key, success)
									TriggerEvent('blckhndr_needs:stress:add', 5)
