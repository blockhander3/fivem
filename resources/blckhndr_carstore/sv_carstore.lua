...
RegisterServerEvent('blckhndr_carstore:floor:Request')
AddEventHandler('blckhndr_carstore:floor:Request', function()
	local i = 1
	...
	TriggerClientEvent('blckhndr_carstore:floor:Update', source, car_spots)
end)

function WorksAtStore(id)
	return exports["blckhndr_jobs"]:isPlayerClockedInWhitelist(id, 1)
end
	...
	if WorksAtStore(source) then
		if split[1] == '/comm' then
			if tonumber(split[2]) and tonumber(split[2]) <= 30 and tonumber(split[2]) > 0 then
				TriggerClientEvent('blckhndr_carstore:floor:commission', source, tonumber(split[2]))
			else
		...
		if split[1] == '/color1' then
			if tonumber(split[2]) and tonumber(split[2]) <= 159 and tonumber(split[2]) > 0 then
				TriggerClientEvent('blckhndr_carstore:floor:color:One', source, tonumber(split[2]))
			else
		...
		if split[1] == '/color2' then
			if tonumber(split[2]) and tonumber(split[2]) <= 159 and tonumber(split[2]) > 0 then
				TriggerClientEvent('blckhndr_carstore:floor:color:Two', source, tonumber(split[2]))
			else
		...
		if split[1] == '/testdrive' then
			if split[2] then
				if split[2] == 'end' then
					TriggerClientEvent('blckhndr_carstore:testdrive:end', source)
				end
			else
				TriggerClientEvent('blckhndr_carstore:testdrive:start', source)
			end
		end
	end
end)
...
RegisterServerEvent('blckhndr_carstore:floor:color:One')
AddEventHandler('blckhndr_carstore:floor:color:One', function(carnum, col)
	car_spots[carnum]['car']['color'][1] = col
	car_spots[carnum]['car']['updated'] = false
	TriggerClientEvent('blckhndr_carstore:floor:UpdateCar', -1, carnum, car_spots[carnum])
	print('Player('..source..') is updating '..car_spots[carnum]['car']['model']..' color1 to '..col)
end)
RegisterServerEvent('blckhndr_carstore:floor:color:Two')
AddEventHandler('blckhndr_carstore:floor:color:Two', function(carnum, col)
	car_spots[carnum]['car']['color'][1] = col
	car_spots[carnum]['car']['updated'] = false
	TriggerClientEvent('blckhndr_carstore:floor:UpdateCar', -1, carnum, car_spots[carnum])
	print('Player('..source..') is updating '..car_spots[carnum]['car']['model']..' color2 to '..col)
end)
RegisterServerEvent('blckhndr_carstore:floor:commission')
AddEventHandler('blckhndr_carstore:floor:commission', function(carnum, amt)
	car_spots[carnum]['car']['commission'] = amt
	car_spots[carnum]['car']['updated'] = false
	TriggerClientEvent('blckhndr_carstore:floor:UpdateCar', -1, carnum, car_spots[carnum])
	print('Player('..source..') is updating '..car_spots[carnum]['car']['model']..' commission to '..amt)
end)
RegisterServerEvent('blckhndr_carstore:floor:ChangeCar')
AddEventHandler('blckhndr_carstore:floor:ChangeCar', function(carnum, mdl)
