...
function BuyCar(key)
	...
	TriggerServerEvent('blckhndr_cargarage:buyVehicle', exports["blckhndr_main"]:blckhndr_CharID(), car_spots[key].car.name, car_spots[key].car.model, GetVehicleNumberPlateText(personalvehicle), details, finance, 'c', 0)
	...
	TriggerEvent('blckhndr_bank:change:walletMinus', car_spots[key].car.buyprice+car_spots[key].car.commission)
	TriggerEvent('blckhndr_cargarage:makeMine', personalvehicle, car_spots[key].car.model, GetVehicleNumberPlateText(personalvehicle))
end
					...
					if not car.updated then
						...
						if GetDistanceBetweenCoords(car.x, car.y, car.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
							...
							if exports["blckhndr_jobs"]:isWhitelistClockedIn(1) then
								...
								if IsControlJustPressed(0, 38) then
									-- [E] Purchase
									if exports["blckhndr_main"]:blckhndr_CanAfford(car.car.buyprice+comm) then
										BuyCar(key)
									else
...
TriggerServerEvent('blckhndr_carstore:floor:Request')
RegisterNetEvent('blckhndr_carstore:floor:Update')
AddEventHandler('blckhndr_carstore:floor:Update', function(tbl)
	car_spots = tbl
end)
RegisterNetEvent('blckhndr_carstore:floor:UpdateCar')
AddEventHandler('blckhndr_carstore:floor:UpdateCar', function(updatedcar, tbl)
	print('blckhndr_carstore: got update for car('..updatedcar..')')
...
-- command shit
RegisterNetEvent('blckhndr_carstore:floor:commission')
AddEventHandler('blckhndr_carstore:floor:commission', function(amt)
	for k,v in pairs(car_spots) do
		if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
			TriggerServerEvent('blckhndr_carstore:floor:commission', k, amt)
		end
	end
end)
RegisterNetEvent('blckhndr_carstore:floor:color:One')
AddEventHandler('blckhndr_carstore:floor:color:One', function(col)
	for k,v in pairs(car_spots) do
		if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
			TriggerServerEvent('blckhndr_carstore:floor:color:One', k, col)
		end
	end
end)
RegisterNetEvent('blckhndr_carstore:floor:color:Two')
AddEventHandler('blckhndr_carstore:floor:color:Two', function(col)
	for k,v in pairs(car_spots) do
		if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then
			TriggerServerEvent('blckhndr_carstore:floor:color:Two', k, col)
		end
	end
end)
...
function TestDriveVehicle(key)
	...
	TriggerEvent('blckhndr_cargarage:makeMine', testvehicle, car_spots[key].car.model, GetVehicleNumberPlateText(testvehicle))
