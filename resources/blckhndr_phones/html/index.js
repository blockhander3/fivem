...
function changePage(page, phone) { // simple function that disables the css from the previous page and enables it for the new one -- will also show/hide all the pages
	...
	for (i = 0; i < document.styleSheets.length; i++) {
		...
		var path = document.styleSheets.item(i).href.replace('nui://blckhndr_phones/html/pages_css/', '');
...
function messageAreaActive() {
	$.post('http://blckhndr_phones/messageactive', JSON.stringify({}));
}

function messageAreaInActive() {
	$.post('http://blckhndr_phones/messageinactive', JSON.stringify({}));
}
*/
...
function updateContact() {
	...
	$.post('http://blckhndr_phones/updateAddContact', JSON.stringify({
		'pn':pn,
		'name':name
	}));
...
function deleteContact() {
	var pn = $('#editcontact_number').val()
	$.post('http://blckhndr_phones/removeContact', JSON.stringify({
		'pn':pn
	}));
	changePage('home', currentPhone);
}
...
function callContact(pn) {
	$.post('http://blckhndr_phones/callContact', JSON.stringify({
		'pn':pn
	}));
}
...
function WLUser(id) {
	...
	$.post('http://blckhndr_phones/whitelistUser', JSON.stringify({
		'charid': parseInt(usr),
		'lvl': parseInt(lvl),
		'wl': wl
	}));
...
document.body.onmouseup = function() {
	...
	if (oldY > newY) {
		...
		if ( diff > buffer) {
			if (currentPage == 'home') {
				$.post('http://blckhndr_phones/closePhone', JSON.stringify({}));
			} else {
				changePage('home', currentPhone);
				//messageAreaInActive()
			}
...
function sendText(number, msg) {
	...
	$.post('http://blckhndr_phones/sendToServer', JSON.stringify({
		'message': {
			'to': number,
			'body': msg
		},
...
function clockInWhitelist(id) {
	log('toggling wl '+id)
	$.post('http://blckhndr_phones/toggleWhitelist', JSON.stringify({
		'id': id
	}));
}
